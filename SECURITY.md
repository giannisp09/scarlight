# Scarlight Agent Security Policy

This document describes Scarlight Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/giannisp09/scarlight/security/advisories/new).
Do not open public issues for
security vulnerabilities. **Scarlight Agent does not operate a bug
bounty program.**
...
- Environment details (`scarlight version`, commit SHA, OS, Python
  version).
...
Scarlight Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Scarlight Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Scarlight Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Scarlight Agent's documentation
...
Scarlight Agent supports two OS-level isolation postures. They address
...
Scarlight Agent supports this in two ways:

- **Scarlight Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Scarlight Agent's in-process heuristics
...
Scarlight Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Scarlight Agent
itself. Bugs in Scarlight Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Scarlight Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Scarlight Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Scarlight Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  scarlight-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Scarlight Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Scarlight Agent. Bugs in Scarlight
...
- Scarlight Agent includes supply-chain guards for MCP server
...
- **Channel:** the private GitHub Security Advisory (GHSA) thread
  opened from the link in §1.
