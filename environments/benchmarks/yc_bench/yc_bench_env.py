...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs ScarlightAgentLoop with terminal tool only
...
class YCBenchEvalConfig(ScarlightAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends ScarlightAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(ScarlightAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The ScarlightAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs ScarlightAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our ScarlightAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the ScarlightAgentLoop
            # ----------------------------------------------------------
