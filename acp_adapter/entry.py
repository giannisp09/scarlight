"""CLI entry point for the scarlight-agent ACP adapter.

Loads environment variables from ``~/.scarlight/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from SCARLIGHT_HOME (default ``~/.scarlight``)."""
    from scarlight_cli.env_loader import load_scarlight_dotenv

    scarlight_home = get_scarlight_home()
    loaded = load_scarlight_dotenv(scarlight_home=scarlight_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", scarlight_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting scarlight-agent ACP adapter")
    ...
    import acp
    from .server import ScarlightACPAgent
