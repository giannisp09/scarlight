"""ACP session manager — maps ACP sessions to Scarlight AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.scarlight/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Scarlight itself is running in WSL.

    Windows ACP clients can launch ``scarlight acp`` inside WSL while still sending
    ...
    """
    from scarlight_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Scarlight from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["scarlight-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Scarlight agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Scarlight AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Scarlight runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.scarlight/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``SCARLIGHT_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_scarlight_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=scarlight_home / "state.db")
            return self._db_instance
        except Exception:
