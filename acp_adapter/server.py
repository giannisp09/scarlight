"""ACP agent server — exposes Scarlight Agent via the Agent Client Protocol."""
...
try:
    from scarlight_cli import __version__ as SCARLIGHT_VERSION
except Exception:
    SCARLIGHT_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Scarlight running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Scarlight cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Scarlight/OpenAI-compatible user content payload."""
...
class ScarlightACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Scarlight AIAgent."""
    ...
    _SLASH_COMMANDS = {
        ...
        "version": "Show Scarlight version",
    }
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Scarlight version",
        },
    )
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from scarlight_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from scarlight_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Scarlight estimates ``used`` from the same
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["scarlight-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Scarlight using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="scarlight-agent", version=SCARLIGHT_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Scarlight' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications; merely restoring server-side state makes Scarlight remember
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Scarlight on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Scarlight' local kawaii waiting/status
        ...
        # not the executor's. Also set SCARLIGHT_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue SCARLIGHT_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind SCARLIGHT_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("SCARLIGHT_INTERACTIVE")
            ...
            try:
                ...
                # Restore SCARLIGHT_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("SCARLIGHT_INTERACTIVE", None)
                else:
                    os.environ["SCARLIGHT_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (SCARLIGHT_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Scarlight Agent v{SCARLIGHT_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Scarlight has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
