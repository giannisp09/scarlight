...
TOOLSETS = {
    ...
    # ==========================================================================
    # Full Scarlight toolsets (CLI + messaging platforms)
    #
    ...
    "scarlight-email": {
        "description": "Email bot toolset - interact with Scarlight via email (IMAP/SMTP)",
        "tools": _SCARLIGHT_CORE_TOOLS,
        "includes": []
    },

    "scarlight-mattermost": {
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (scarlight-<name>).
        # Gives them _SCARLIGHT_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("scarlight-"):
            platform_name = name[len("scarlight-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SCARLIGHT_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
