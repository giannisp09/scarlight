...
There are two ways to add a platform to the Scarlight gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.scarlight/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Scarlight code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `scarlight gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `scarlight_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Scarlight core.
...
And add it to the `scarlight-gateway` composite:

```python
"scarlight-gateway": {
    "includes": [..., "scarlight-your-platform"]
}
```
...
## 12. Status Display (`scarlight_cli/status.py`)
...
## 13. Gateway Setup Wizard (`scarlight_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ scarlight_cli/ toolsets.py \
  --include="*.py" -l | sort -u
