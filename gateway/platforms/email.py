"""
Email platform adapter for the Scarlight gateway.

Allows users to interact with Scarlight by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _scarlight_version = "0"
        imap.xatom(
            "ID",
            f'("name" "scarlight-agent" "version" "{_scarlight_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Scarlight Agent")
        ...
        msg_id = f"<scarlight-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
