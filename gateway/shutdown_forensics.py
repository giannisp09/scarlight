    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if scarlight_home_str:
            takeover_path = Path(scarlight_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(scarlight_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded scarlight-agent but never re-ran ``scarlight setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../scarlight-gateway.service"
    ...
    # that's the common case for scarlight.
    ...
    # constant in scarlight_cli/gateway.py.
    headroom = 30.0
