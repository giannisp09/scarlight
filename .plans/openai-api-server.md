# OpenAI-Compatible API Server for Scarlight Agent
...
SSE streaming. By exposing this endpoint, scarlight-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  scarlight-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.scarlight/.env`
2. Run `scarlight gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with scarlight-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns scarlight-agent as a model) |
...
Authorization: Bearer scarlight-api-key-here
...
But scarlight-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "scarlight-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured scarlight-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of scarlight-agent's core prompt
...
Once implemented, scarlight-agent works as a drop-in backend for:
