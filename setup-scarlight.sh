...
# 5. Symlinks the 'scarlight' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Scarlight Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/scarlight-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink scarlight into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up scarlight command..."

SCARLIGHT_BIN="$SCRIPT_DIR/venv/bin/scarlight"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SCARLIGHT_BIN" "$COMMAND_LINK_DIR/scarlight"
echo -e "${GREEN}✓${NC} Symlinked scarlight → $COMMAND_LINK_DISPLAY_DIR/scarlight"
...
# ============================================================================
# Seed bundled skills into ~/.scarlight/skills/
# ============================================================================

SCARLIGHT_SKILLS_DIR="${SCARLIGHT_HOME:-$HOME/.scarlight}/skills"
mkdir -p "$SCARLIGHT_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.scarlight/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SCARLIGHT_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  scarlight status        # Check configuration"
if is_termux; then
    echo "  scarlight gateway       # Run gateway in foreground"
else
    echo "  scarlight gateway install # Install gateway service (messaging + cron)"
fi
echo "  scarlight cron list     # View scheduled jobs"
echo "  scarlight doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m scarlight_cli.main setup
fi
