...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.scarlight/.env ] && grep -q "^GITHUB_TOKEN=" ~/.scarlight/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.scarlight/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${SCARLIGHT_HOME:-$HOME/.scarlight}/skills/github/github-auth/scripts/gh-env.sh"
