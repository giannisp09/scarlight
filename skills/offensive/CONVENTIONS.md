...
Two sandbox images are relevant:

- **Default (`DEFAULT_TERMINAL_IMAGE` = `kalilinux/kali-last-release`, digest-pinned).** Minimal by design (~120MB); ships almost none of the tools below. The agent apt-installs each on first use. Fine for light/passive work; slow and noisy for a full kill-chain.
- **`scarlight-kali` (opt-in, recommended for exploitation).** Built from [`docker/kali/`](../../docker/kali/) — the default base plus the whole v1.1 toolset baked in, with the target-delivered binaries staged in `$SCARLIGHT_PAYLOAD_DIR` (`/opt/scarlight/payloads`). Build with `docker/kali/build.sh`, then `export TERMINAL_DOCKER_IMAGE=scarlight-kali:latest` (or set `terminal.docker_image`). This is what lets linpeas/winpeas/mimikatz be served target-ward from the sandbox instead of curled on the target.

The image is expected to contain: `sqlmap`, `commix`, `hashcat`, `john`, `hydra`, `metasploit-framework`, `exploitdb` (`searchsploit`), `impacket-*` (including `impacket-psexec`, `impacket-wmiexec`, `impacket-secretsdump`, `impacket-ticketConverter`), `keepass2john`, `nmap`, `gobuster`, `curl`, `openssl`, `dig`, `whois`, `jq`, `ssh`. On the minimal default these arrive via first-use `apt`; on `scarlight-kali` they are pre-baked.

Tools NOT in the base Kali image — pre-baked in `scarlight-kali` (see `docker/kali/Dockerfile`), or fetched on first use on the minimal default (see `specs/exploitation-v1/requirements.md` §5.2):
