"""
Scarlight MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Scarlight-specific extra)

Usage:
    scarlight mcp serve
    scarlight mcp serve --verbose
...
logger = logging.getLogger("scarlight.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SCARLIGHT_HOME."""
    try:
        from scarlight_constants import get_scarlight_home
        return get_scarlight_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SCARLIGHT_HOME", Path.home() / ".scarlight")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from scarlight_constants import get_scarlight_home
        directory_file = get_scarlight_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SCARLIGHT_HOME", Path.home() / ".scarlight")
...
class EventBridge:
    ...
    This is the Scarlight equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from scarlight_constants import get_scarlight_home
            db_file = get_scarlight_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SCARLIGHT_HOME", Path.home() / ".scarlight")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Scarlight MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "scarlight",
        instructions=(
            "Scarlight Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Scarlight MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
