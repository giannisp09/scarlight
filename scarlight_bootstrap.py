"""Windows UTF-8 bootstrap for Scarlight entry points.
...
should be imported at the very top of every Scarlight entry point
(``scarlight``, ``scarlight-agent``, ``scarlight-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import scarlight_bootstrap``
# (or ``from scarlight_bootstrap import apply_windows_utf8_bootstrap``) at
