"""Persistent slash-command worker — one ScarlightCLI per TUI session.
...
def _run(cli: ScarlightCLI, command: str) -> str:
...
def main():
    ...
    os.environ["SCARLIGHT_SESSION_KEY"] = args.session_key
    os.environ["SCARLIGHT_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ScarlightCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
