# nix/nixosModules.nix — NixOS module for scarlight
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: scarlight runs from /nix/store bind-mounted read-only into a
...
# and read by scarlight at startup — no container recreation needed for env changes.
#
# Tool resolution: the scarlight wrapper uses --suffix PATH for nix store tools,
...
#     environmentFiles = [ config.sops.secrets."scarlight/env".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    ...
    scarlight = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-hermes-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "scarlight-config-attrs";
      description = "Scarlight YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "scarlight-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "scarlight-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'SCARLIGHT_DOC_EOF'\n${value}\nSCARLIGHT_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "scarlight-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/scarlight";
    ...
    # scarlight user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "scarlight-container-entrypoint" ''
      set -eu

      SCARLIGHT_UID="''${SCARLIGHT_UID:?SCARLIGHT_UID must be set}"
      SCARLIGHT_GID="''${SCARLIGHT_GID:?SCARLIGHT_GID must be set}"

      # ── Group: ensure a group with GID=$SCARLIGHT_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$SCARLIGHT_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="scarlight"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$SCARLIGHT_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$SCARLIGHT_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$SCARLIGHT_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$SCARLIGHT_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="scarlight"
        TARGET_HOME="/home/scarlight"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$SCARLIGHT_UID" -g "$SCARLIGHT_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$SCARLIGHT_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$SCARLIGHT_UID:$SCARLIGHT_GID" "$TARGET_HOME"
      ...
      if [ -n "''${SCARLIGHT_HOME:-}" ] && [ -d "$SCARLIGHT_HOME" ]; then
        find "$SCARLIGHT_HOME" \! -user "$SCARLIGHT_UID" -exec chown "$SCARLIGHT_UID:$SCARLIGHT_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/scarlight-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/scarlight-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/scarlight ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/scarlight
        chmod 0440 /etc/sudoers.d/scarlight
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$SCARLIGHT_UID" --regid="$SCARLIGHT_GID" --init-groups "$@"
    ...
    # Default: /var/lib/scarlight/workspace → /data/workspace.
  ...
  in {
    options.services.scarlight-agent = with lib; {
      enable = mkEnableOption "Scarlight Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = scarlight;
        description = "The scarlight package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Scarlight config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Scarlight reads this file on every startup via load_scarlight_dotenv().
        '';
      };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `scarlight gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the scarlight user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the scarlight plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Scarlight discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as scarlight (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in scarlight's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the scarlight CLI to environment.systemPackages and export
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          description = ''
            ...
            stateDir. These users are automatically added to the scarlight group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the scarlight CLI to system PATH and export SCARLIGHT_HOME system-wide
      ...
      # Wire extraPackages into the scarlight user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            ...
            is set, but addToSystemPackages is false. Without a host-installed scarlight
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure scarlight is on PATH.
          ''
        ];
      })
      ...
      {
        system.activationScripts."scarlight-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.scarlight/.container-mode <<'SCARLIGHT_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    scarlight_bin=${containerDataDir}/current-package/bin/scarlight
    SCARLIGHT_CONTAINER_MODE_EOF
            ...
            ${lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.scarlight" ]; then
                  rm -f "${symlinkPath}"
                  echo "scarlight: removed symlink ${symlinkPath}"
                fi
            ...
            (lib.concatStringsSep "\n" (map (user:
              ...
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  ...
                  echo "scarlight: backing up existing ${symlinkPath} to $_backup"
          ...
          # Scarlight reads $SCARLIGHT_HOME/.env at startup via load_scarlight_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.scarlight/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'SCARLIGHT_NIX_ENV_EOF'
    ${envFileContent}
    SCARLIGHT_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.scarlight-agent = {
          description = "Scarlight Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $SCARLIGHT_HOME/.env by the activation script. load_scarlight_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/scarlight"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the scarlight group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.scarlight-agent = {
          description = "Scarlight Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              SCARLIGHT_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              SCARLIGHT_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env SCARLIGHT_UID="$SCARLIGHT_UID" \
                --env SCARLIGHT_GID="$SCARLIGHT_GID" \
                --env SCARLIGHT_HOME=${containerDataDir}/.scarlight \
                --env SCARLIGHT_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/scarlight gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
