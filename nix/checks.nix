...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      scarlight = self'.packages.default;
      scarlightVenv = scarlight.scarlightVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "scarlight-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${scarlightVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "scarlight-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "scarlight-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${scarlight}/bin/scarlight || (echo "FAIL: scarlight binary missing"; exit 1)
          test -x ${scarlight}/bin/scarlight-agent || (echo "FAIL: scarlight binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${scarlight}/bin/scarlight version 2>&1 | grep -qi "scarlight" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "scarlight-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in scarlight scarlight-agent scarlight-acp; do
            test -x ${scarlight}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "scarlight-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking scarlight --help ==="
          ${scarlight}/bin/scarlight --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${scarlight}/bin/scarlight --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "scarlight-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${scarlight}/share/scarlight/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${scarlight}/share/scarlight/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SCARLIGHT_BUNDLED_SKILLS" ${scarlight}/bin/scarlight || \
            (echo "FAIL: SCARLIGHT_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SCARLIGHT_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "scarlight-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${scarlight}/share/scarlight/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${scarlight}/share/scarlight/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SCARLIGHT_BUNDLED_PLUGINS" ${scarlight}/bin/scarlight || \
            (echo "FAIL: SCARLIGHT_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SCARLIGHT_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "scarlight-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${scarlight}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${scarlight}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SCARLIGHT_TUI_DIR" ${scarlight}/bin/scarlight || \
            (echo "FAIL: SCARLIGHT_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SCARLIGHT_TUI_DIR set in wrapper"
        ...
        # Verify SCARLIGHT_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        scarlight-node = pkgs.runCommand "scarlight-node-version" { } ''
          set -e
          echo "=== Checking SCARLIGHT_NODE in wrapper ==="
          grep -q "SCARLIGHT_NODE" ${scarlight}/bin/scarlight || \
            (echo "FAIL: SCARLIGHT_NODE not set in wrapper"; exit 1)
          echo "PASS: SCARLIGHT_NODE present in wrapper"

          SCARLIGHT_NODE=$(sed -n "s/^export SCARLIGHT_NODE='\(.*\)'/\1/p" ${scarlight}/bin/scarlight)
          test -x "$SCARLIGHT_NODE" || (echo "FAIL: SCARLIGHT_NODE=$SCARLIGHT_NODE not executable"; exit 1)
          echo "PASS: SCARLIGHT_NODE executable at $SCARLIGHT_NODE"

          NODE_MAJOR=$("$SCARLIGHT_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All SCARLIGHT_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SCARLIGHT_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "scarlight-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SCARLIGHT_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SCARLIGHT_MANAGED guards ==="
          check_blocked "config set" ${scarlight}/bin/scarlight config set model foo
          check_blocked "config edit" ${scarlight}/bin/scarlight config edit
        ...
        extra-python-packages = let
          ...
          scarlightWithExtra = scarlight.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "scarlight-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${scarlightWithExtra}/bin/scarlight || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${scarlightWithExtra}/bin/scarlight || \
          ...
          if grep -q "PYTHONPATH" ${scarlight}/bin/scarlight; then
        ...
        extra-dependency-groups = let
          scarlightWithGroups = scarlight.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "scarlight-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${scarlightWithGroups}"
          echo "venv: ${scarlightWithGroups.scarlightVenv}"
        ...
        in pkgs.runCommand "scarlight-config-roundtrip" {
          ...
          merge_and_load() {
            local scarlight_home="$1"
            export SCARLIGHT_HOME="$scarlight_home"
            ${configMergeScript} ${nixSettings} "$scarlight_home/config.yaml"
            ${scarlightVenv}/bin/python3 -c '
import json, sys
