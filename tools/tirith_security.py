...
it is automatically downloaded from GitHub releases to $SCARLIGHT_HOME/bin/tirith.
...
def _get_scarlight_home() -> str:
    """Return the Scarlight home directory, respecting SCARLIGHT_HOME env var."""
    return str(get_scarlight_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_scarlight_home(), ".tirith-install-failed")
...
def _scarlight_bin_dir() -> str:
    """Return $SCARLIGHT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_scarlight_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SCARLIGHT_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_scarlight_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SCARLIGHT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SCARLIGHT_HOME/bin/tirith
    ...
    scarlight_bin = os.path.join(_scarlight_bin_dir(), "tirith")
    if os.path.isfile(scarlight_bin) and os.access(scarlight_bin, os.X_OK):
        _resolved_path = scarlight_bin
        _install_failure_reason = ""
        _clear_install_failed()
