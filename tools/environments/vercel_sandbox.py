...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Scarlight'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``SCARLIGHT_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_scarlight_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            container_base = "/.scarlight"
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.scarlight"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_scarlight = (
            "/.scarlight"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.scarlight"
        )
        archive_member = remote_scarlight.lstrip("/")
        remote_tar = f"/tmp/.scarlight_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
