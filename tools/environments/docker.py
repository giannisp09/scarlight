...
def _load_scarlight_env_vars() -> dict[str, str]:
    """Load ~/.scarlight/.env values without failing Docker command execution."""
    try:
        from scarlight_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``SCARLIGHT_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("SCARLIGHT_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using SCARLIGHT_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'scarlight'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.scarlight/sandboxes/). Non-persistent
        ...
        container_name = f"scarlight-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Scarlight secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _SCARLIGHT_PROVIDER_ENV_BLOCKLIST)
        scarlight_env = _load_scarlight_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = scarlight_env.get(key)
