...
This module provides the glue:
    - ``ScarlightTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Scarlight Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses SCARLIGHT_HOME so each profile gets its own OAuth tokens.
    Layout: ``SCARLIGHT_HOME/mcp-tokens/``
    """
    try:
        from scarlight_constants import get_scarlight_home
        base = Path(get_scarlight_home())
    except ImportError:
        base = Path(os.environ.get("SCARLIGHT_HOME", str(Path.home() / ".scarlight")))
...
# ---------------------------------------------------------------------------
# ScarlightTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ScarlightTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        SCARLIGHT_HOME/mcp-tokens/<server_name>.json         -- tokens
        SCARLIGHT_HOME/mcp-tokens/<server_name>.client.json   -- client info
        SCARLIGHT_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Scarlight records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Scarlight.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Scarlight Agent")
    scope = cfg.get("scope")
