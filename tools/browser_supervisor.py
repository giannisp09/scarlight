...
One ``CDPSupervisor`` runs per Scarlight ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "scarlight-dialog-bridge.invalid"
...
(() => {
  if (window.__scarlightDialogBridgeInstalled) return;
  window.__scarlightDialogBridgeInstalled = true;
  const ENDPOINT = "http://scarlight-dialog-bridge.invalid/";
