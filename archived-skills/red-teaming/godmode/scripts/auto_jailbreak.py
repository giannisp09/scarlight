...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SCARLIGHT_HOME", os.path.expanduser("~/.scarlight")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SCARLIGHT_HOME", Path.home() / ".scarlight")) / "skills" / "red-teaming" / "godmode"
...
SCARLIGHT_HOME = Path(os.getenv("SCARLIGHT_HOME", Path.home() / ".scarlight"))
CONFIG_PATH = SCARLIGHT_HOME / "config.yaml"
PREFILL_PATH = SCARLIGHT_HOME / "prefill.json"
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.scarlight/prefill.json."""
