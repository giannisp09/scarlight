...
2. User-installed providers: ``$SCARLIGHT_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$SCARLIGHT_HOME/plugins/`` or None if unavailable."""
    try:
        from scarlight_constants import get_scarlight_home
        d = get_scarlight_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$SCARLIGHT_HOME/plugins/<name>/``) directories.  Bundled takes
