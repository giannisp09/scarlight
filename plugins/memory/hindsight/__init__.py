...
Or via $SCARLIGHT_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $SCARLIGHT_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_scarlight_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "scarlight": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "scarlight"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    ...
    profile = config.get("profile", "scarlight")
    return str(profile or "scarlight")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``scarlight-{user}`` with no user becomes ``hermes``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Scarlight Agent and the User"
    ...
    def save_config(self, values, hermes_home):
        """Write config to $SCARLIGHT_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        from scarlight_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "scarlight")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "scarlight"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: scarlight-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Scarlight Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "scarlight"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "scarlight"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "scarlight", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "scarlight")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Scarlight Agent and the User")
