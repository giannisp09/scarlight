...
echo "HINDSIGHT_API_KEY=your-key" >> ~/.scarlight/.env
```
...
Daemon startup logs: `~/.scarlight/logs/hindsight-embed.log`
...
Config file: `~/.scarlight/hindsight/config.json`
...
The LLM API key is stored in `~/.scarlight/.env` as `HINDSIGHT_LLM_API_KEY`.
