...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SCARLIGHT_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _hermes_home = _get_scarlight_home()
        except (ModuleNotFoundError, ImportError):
            _hermes_home = _Path.home() / ".scarlight"
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SCARLIGHT_HOME", str(_Path.home() / ".scarlight"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SCARLIGHT_HOME}/google_chat_user_tokens/<sanitized>.json
...
def interactive_setup() -> None:
    ...
    The setup wizard at ``scarlight_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.scarlight/.env`` so the next
    gateway restart picks them up.
    """
    from scarlight_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print()
    print_success("Google Chat configuration saved to ~/.scarlight/.env")
