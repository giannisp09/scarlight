...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SCARLIGHT_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            hermes_home = Path(get_scarlight_home()).resolve()
        except Exception:
            hermes_home = Path.home().joinpath(".scarlight").resolve()
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.scarlight/.env`` via ``scarlight_cli.config``.
    """
    print()
    ...
    try:
        from scarlight_cli.config import get_env_var, set_env_var
    except ImportError:
        print("scarlight_cli.config not available; set LINE_* vars manually in ~/.scarlight/.env")
        return
