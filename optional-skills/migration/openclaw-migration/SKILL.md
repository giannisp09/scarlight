...
It uses `scripts/openclaw_to_scarlight.py` to:
...
- copy OpenClaw skills into `~/.scarlight/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Hermes workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.scarlight/tts/`
...
- `scripts/openclaw_to_scarlight.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.scarlight/skills/migration/openclaw-migration/scripts/openclaw_to_scarlight.py`

Do not guess a shorter path like `~/.scarlight/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.scarlight/skills/migration/openclaw-migration/`.
...
```bash
python3 ~/.scarlight/skills/migration/openclaw-migration/scripts/openclaw_to_scarlight.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.scarlight/skills/migration/openclaw-migration/scripts/openclaw_to_scarlight.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.scarlight/skills/migration/openclaw-migration/scripts/openclaw_to_scarlight.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.scarlight/skills/migration/openclaw-migration/scripts/openclaw_to_scarlight.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.scarlight/skills/migration/openclaw-migration/scripts/openclaw_to_scarlight.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.scarlight/skills/migration/openclaw-migration/scripts/openclaw_to_scarlight.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
- OpenClaw skills available under `~/.scarlight/skills/openclaw-imports/`
