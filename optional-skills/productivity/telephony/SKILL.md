...
- save provider credentials into `~/.scarlight/.env`
...
### `~/.scarlight/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.scarlight/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `~/.scarlight/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.scarlight/.env`
