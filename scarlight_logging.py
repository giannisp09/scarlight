"""Centralized logging setup for Scarlight Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.scarlight/logs/`` (profile-aware via ``get_scarlight_home()``).
...
from scarlight_constants import get_config_path, get_scarlight_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_scarlight_session_injector", False):
    ...
    _session_record_factory._scarlight_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``scarlight logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("scarlight_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    *,
    ...
    """Configure the Scarlight logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    scarlight_home
        Override for the Scarlight home directory.  Falls back to
        ``get_scarlight_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_scarlight_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._scarlight_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the scarlight group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from scarlight_cli.config import is_managed
