...
Validates that:
1. The installed atroposlib API still matches Scarlight's expectations
2. Scarlight's parser registry remains compatible with ManagedServer parsing
3. ScarlightAgentBaseEnv wires the selected parser into ServerManager correctly

These tests verify the contract between scarlight-agent's environments/ code
...
class TestManagedServerAPI:
    """Test that ManagedServer's API matches what scarlight-agent expects."""
...
class TestParserCompatibility:
    """Test that scarlight-agent's parsers match ManagedServer's expectations."""
...
class TestBaseEnvCompatibility:
    ...
    def test_hermes_base_env_sets_server_manager_tool_parser(self):
        """Scarlight wires parser selection through ServerManager.tool_parser."""
        import ast
