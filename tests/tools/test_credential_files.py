...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        (scarlight_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(scarlight_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.scarlight/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        (scarlight_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        (scarlight_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        (scarlight_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        scarlight_home = tmp_path / ".scarlight"
        skills_dir = scarlight_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.scarlight/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        scarlight_home = tmp_path / ".scarlight"
        (scarlight_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SCARLIGHT_HOME": str(scarlight_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.scarlight")

        assert mounts[0]["container_path"] == "/home/user/.scarlight/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.scarlight/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.scarlight/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SCARLIGHT_HOME."""
        monkeypatch.setenv("SCARLIGHT_HOME", str(tmp_path / ".scarlight"))
        (tmp_path / ".scarlight").mkdir()

        # Create a sensitive file one level above scarlight_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside scarlight_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside SCARLIGHT_HOME must still be registered."""
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))
        (scarlight_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_scarlight_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SCARLIGHT_HOME must be allowed."""
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        subdir = scarlight_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SCARLIGHT_HOME pointing outside must be rejected."""
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))

        # Create a sensitive file outside scarlight_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside scarlight_home pointing outside
        symlink = scarlight_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SCARLIGHT_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, scarlight_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SCARLIGHT_HOME."""
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))
        ...
        sensitive.write_text("{}")
        self._write_config(scarlight_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(scarlight_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SCARLIGHT_HOME via config must still mount."""
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))

        (scarlight_home / "oauth.json").write_text("{}")
        self._write_config(scarlight_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        (scarlight_home / "cache" / "documents").mkdir(parents=True)
        (scarlight_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.scarlight/cache/documents" in paths
        assert "/root/.scarlight/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.scarlight/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        # Use legacy dir name — get_scarlight_dir prefers old if it exists
        (scarlight_home / "document_cache").mkdir()
        (scarlight_home / "image_cache").mkdir()
        monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(scarlight_home / "document_cache") in host_paths
        assert str(scarlight_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.scarlight/cache/documents" in container_paths
        assert "/root/.scarlight/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        scarlight_home = tmp_path / ".scarlight"
        doc_dir = scarlight_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        scarlight_home = tmp_path / ".scarlight"
        ss_dir = scarlight_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.scarlight/cache/screenshots/session_abc/screen1.png"
