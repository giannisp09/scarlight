...
def test_scarlight_provider_subclass_exists():
    """ScarlightMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _SCARLIGHT_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _SCARLIGHT_PROVIDER_CLS is not None
    assert issubclass(_SCARLIGHT_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_scarlight_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ScarlightMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _SCARLIGHT_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("SCARLIGHT_HOME", str(tmp_path))
    ...
    assert _SCARLIGHT_PROVIDER_CLS is not None
    assert isinstance(provider, _SCARLIGHT_PROVIDER_CLS)
    assert provider._scarlight_server_name == "srv"
