...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._scarlight_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_scarlight_bin(self):
        """After _INSTALL_FAILED, manual install in SCARLIGHT_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        scarlight_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(scarlight_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(scarlight_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._scarlight_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == scarlight_bin
            assert _tirith_mod._resolved_path == scarlight_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# SCARLIGHT_HOME isolation
# ---------------------------------------------------------------------------

class TestScarlightHomeIsolation:
    def test_scarlight_bin_dir_respects_scarlight_home(self):
        """_scarlight_bin_dir must use SCARLIGHT_HOME, not hardcoded ~/.scarlight."""
        from tools.tirith_security import _scarlight_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"SCARLIGHT_HOME": tmpdir}):
            result = _scarlight_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_scarlight_home(self):
        """_failure_marker_path must use SCARLIGHT_HOME, not hardcoded ~/.scarlight."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"SCARLIGHT_HOME": "/custom/scarlight"}):
            result = _failure_marker_path()
        assert result == "/custom/scarlight/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets SCARLIGHT_HOME; verify it's active."""
        scarlight_home = os.getenv("SCARLIGHT_HOME")
        assert scarlight_home is not None, "SCARLIGHT_HOME should be set by conftest"
        assert "scarlight_test" in scarlight_home, "Should point to test temp dir"

    def test_get_scarlight_home_fallback(self):
        """Without SCARLIGHT_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_scarlight_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove SCARLIGHT_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".scarlight")
            result = _get_scarlight_home()
