...
def _clean_state():
    ...
    saved = {}
    for k in ("SCARLIGHT_INTERACTIVE", "SCARLIGHT_GATEWAY_SESSION", "SCARLIGHT_EXEC_ASK", "SCARLIGHT_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SCARLIGHT_INTERACTIVE"] = "1"
        session_key = os.getenv("SCARLIGHT_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SCARLIGHT_INTERACTIVE or SCARLIGHT_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
