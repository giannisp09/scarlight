...
def _restore_tool_modules():
    original_scarlight_home = os.environ.get("SCARLIGHT_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "scarlight_cli"
        or name.startswith("scarlight_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_scarlight_home is None:
            os.environ.pop("SCARLIGHT_HOME", None)
        else:
            os.environ["SCARLIGHT_HOME"] = original_scarlight_home
        _reset_modules(("tools", "scarlight_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    scarlight_cli = types.ModuleType("scarlight_cli")
    scarlight_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["scarlight_cli"] = scarlight_cli
    scarlight_home = tmp_path / "scarlight-home"
    os.environ["SCARLIGHT_HOME"] = str(scarlight_home)
    sys.modules["scarlight_cli.config"] = types.SimpleNamespace(
        get_scarlight_home=lambda: scarlight_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="scarlight-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": scarlight_home / "modal_snapshots.json",
