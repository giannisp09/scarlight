...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "scarlight_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_board_auto_initializes_missing_db(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("SCARLIGHT_HOME", str(home))
    monkeypatch.delenv("SCARLIGHT_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("SCARLIGHT_KANBAN_DB", raising=False)
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "scarlight_cli.web_server", stub)
    monkeypatch.setattr(scarlight_cli, "web_server", stub, raising=False)
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "scarlight_cli.kanban._check_dispatcher_presence",
        lambda: (False, "No gateway is running — start `scarlight gateway start`."),
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "scarlight_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_board_surfaces_warnings_field_for_hallucinated_completions(client):
    ...
    raw event kind — see scarlight_cli.kanban_diagnostics for the rule
    that produces it.
    """
    conn = kb.connect()
    try:
...
def test_dashboard_bulk_actions_include_reclaim_first():
    ...
    assert "scarlight-kanban-bulk-reclaim-first" in dist
    assert '"→ todo"' in dist
...
def test_dashboard_failed_card_highlight_class_exists():
    ...
    assert "scarlight-kanban-card--failed" in js
    assert "scarlight-kanban-card--failed" in css
