...
- Path resolution (absolute, relative to SCARLIGHT_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp SCARLIGHT_HOME."""
    scarlight_home = tmp_path / ".scarlight"
    scarlight_home.mkdir()
    (scarlight_home / "cron").mkdir()
    (scarlight_home / "cron" / "output").mkdir()
    (scarlight_home / "scripts").mkdir()
    monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "SCARLIGHT_DIR", scarlight_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", scarlight_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", scarlight_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", scarlight_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.scarlight/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("SCARLIGHT_SESSION_PLATFORM") is None
        assert os.environ.get("SCARLIGHT_SESSION_CHAT_ID") is None
        assert os.environ.get("SCARLIGHT_SESSION_CHAT_NAME") is None
