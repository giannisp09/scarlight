...
class TestScarlightConstantsFallback:
    """Tests for _scarlight_home.py fallback when scarlight_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_scarlight_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _scarlight_home.py with scarlight_constants blocked."""
        monkeypatch.setitem(sys.modules, "scarlight_constants", None)
        spec = importlib.util.spec_from_file_location("_scarlight_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_scarlight_home_env_var(self, monkeypatch, tmp_path):
        """When scarlight_constants is missing, SCARLIGHT_HOME comes from env var."""
        monkeypatch.setenv("SCARLIGHT_HOME", str(tmp_path / "custom-scarlight"))
        module = self._load_helper(monkeypatch)
        assert module.get_scarlight_home() == tmp_path / "custom-scarlight"

    def test_fallback_defaults_to_dot_scarlight(self, monkeypatch):
        """When scarlight_constants is missing and SCARLIGHT_HOME unset, default to ~/.scarlight."""
        monkeypatch.delenv("SCARLIGHT_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_scarlight_home() == Path.home() / ".scarlight"

    def test_fallback_ignores_empty_scarlight_home(self, monkeypatch):
        """Empty/whitespace SCARLIGHT_HOME is treated as unset."""
        monkeypatch.setenv("SCARLIGHT_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_scarlight_home() == Path.home() / ".scarlight"

    def test_fallback_display_scarlight_home_shortens_path(self, monkeypatch):
        """Fallback display_scarlight_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("SCARLIGHT_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_scarlight_home() == "~/.scarlight"

    def test_fallback_display_scarlight_home_profile_path(self, monkeypatch):
        """Fallback display_scarlight_home() handles profile paths under ~/."""
        monkeypatch.setenv("SCARLIGHT_HOME", str(Path.home() / ".scarlight/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_scarlight_home() == "~/.scarlight/profiles/coder"

    def test_fallback_display_scarlight_home_custom_path(self, monkeypatch):
        """Fallback display_scarlight_home() returns full path for non-home locations."""
        monkeypatch.setenv("SCARLIGHT_HOME", "/opt/scarlight-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_scarlight_home() == "/opt/scarlight-custom"

    def test_delegates_to_scarlight_constants_when_available(self):
        """When scarlight_constants IS importable, _scarlight_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_scarlight_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import scarlight_constants
        assert module.get_scarlight_home is scarlight_constants.get_scarlight_home
        assert module.display_scarlight_home is scarlight_constants.display_scarlight_home
