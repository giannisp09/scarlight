...
minimal ``ScarlightCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def scarlight_home(tmp_path, monkeypatch):
    """Isolated SCARLIGHT_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".scarlight"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("SCARLIGHT_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves SCARLIGHT_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal ScarlightCLI stub with an active goal wired in."""
    from cli import ScarlightCLI
    from scarlight_cli.goals import GoalManager

    cli = ScarlightCLI.__new__(ScarlightCLI)
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, scarlight_home):
        ...
        with patch("scarlight_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, scarlight_home):
        ...
        with patch("scarlight_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, scarlight_home):
    ...
    def test_no_assistant_message_skipped(self, scarlight_home):
...
class TestHealthyTurnStillRuns:
    def test_clean_response_enqueues_continuation_when_judge_says_continue(
        ...
        with patch(
            "scarlight_cli.goals.judge_goal",
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, scarlight_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, scarlight_home):
        ...
        src = inspect.getsource(ScarlightCLI.chat)
