...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("scarlight/scarlight-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned scarlight/* and pr-* branches."""

    def test_prunes_orphaned_scarlight_branch(self, git_repo):
        """scarlight/scarlight-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "scarlight/scarlight-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "scarlight/scarlight-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "scarlight/scarlight-deadbeef" in result.stdout
        ...
        assert "scarlight/scarlight-deadbeef" in orphaned
        ...
        assert "scarlight/scarlight-deadbeef" not in result.stdout
