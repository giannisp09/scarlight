...
def _isolate(tmp_path, monkeypatch):
    """Isolate SCARLIGHT_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal ScarlightCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ScarlightCLI.__new__(ScarlightCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["scarlight-core"]
        obj.compact = False
