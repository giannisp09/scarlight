...
def backup_env(monkeypatch, tmp_path):
    """Isolate SCARLIGHT_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_scarlight_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd scarlight_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under SCARLIGHT_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level SCARLIGHT_DIR picks up the tmp HOME."""
    import scarlight_constants
    importlib.reload(scarlight_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.scarlight/cron/jobs.json
