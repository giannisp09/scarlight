...
def test_scarlight_client_tag_includes_current_version():
    """The client tag must reflect scarlight_cli.__version__ verbatim."""
    from scarlight_cli import __version__
    from agent.portal_tags import scarlight_client_tag

    assert scarlight_client_tag() == f"client=scarlight-client-v{__version__}"


def test_scarlight_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import scarlight_client_tag

    tag = scarlight_client_tag()
    assert tag.startswith("client=scarlight-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import scarlight_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=scarlight-agent" in tags
    assert scarlight_client_tag() in tags
    assert len(tags) == 2
