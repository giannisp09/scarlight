...
""",
        ...
        from scarlight_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        (scarlight_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))
        from scarlight_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from scarlight_cli.plugins import PluginContext, PluginManifest, PluginManager
