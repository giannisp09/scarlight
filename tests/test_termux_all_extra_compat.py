...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"scarlight[termux]"' in text
    assert '"scarlight[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"scarlight[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
