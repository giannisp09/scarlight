"""
Tests for mcp_serve — Scarlight MCP server.
...
def _isolate_scarlight_home(tmp_path, monkeypatch):
    """Redirect SCARLIGHT_HOME to a temp directory."""
    monkeypatch.setenv("SCARLIGHT_HOME", str(tmp_path))
    try:
        import scarlight_constants
        monkeypatch.setattr(scarlight_constants, "get_scarlight_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking scarlight_state schema."""
