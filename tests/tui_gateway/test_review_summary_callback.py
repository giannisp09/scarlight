...
def server():
    with patch.dict(
        "sys.modules",
        {
            "scarlight_constants": MagicMock(
                get_scarlight_home=MagicMock(return_value="/tmp/scarlight_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'scarlight-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'scarlight-release' patched"
    }
