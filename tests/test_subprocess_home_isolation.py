...
class TestGetSubprocessHome:
    """Unit tests for scarlight_constants.get_subprocess_home()."""

    def test_returns_none_when_scarlight_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir()
        profile_home = scarlight_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))
        from scarlight_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".scarlight" / "profiles"
        ...
        monkeypatch.setenv("SCARLIGHT_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("SCARLIGHT_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(scarlight_home / "home")
    ...
    def test_no_injection_when_scarlight_home_unset(self, monkeypatch):
        monkeypatch.delenv("SCARLIGHT_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from scarlight_cli.profiles import _PROFILE_DIRS
