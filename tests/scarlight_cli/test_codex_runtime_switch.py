...
class TestApply:
    ...
    def test_enable_succeeds_when_codex_present(self):
        ...
        assert "via MCP" in r.message  # scarlight-tools callback message
        assert cfg["model"]["openai_runtime"] == "codex_app_server"
    ...
    def test_enable_triggers_mcp_migration(self):
        """Enabling codex_app_server should auto-migrate Scarlight mcp_servers
        ...
        with patch.object(crs, "check_codex_binary_ok",
                          return_value=(True, "0.130.0")), \
             patch("scarlight_cli.codex_runtime_plugin_migration.migrate") as mig:
            mig.return_value.migrated = ["filesystem", "scarlight-tools"]
        ...
        # User MCP servers are reported (excluding internal scarlight-tools)
        ...
        # Scarlight tool callback announcement
    ...
    def test_disable_does_not_trigger_migration(self):
        ...
        with patch("scarlight_cli.codex_runtime_plugin_migration.migrate") as mig:
            r = crs.apply(cfg, "auto")
        assert r.success
    ...
    def test_migration_failure_does_not_block_enable(self):
        ...
        with patch.object(crs, "check_codex_binary_ok",
                          return_value=(True, "0.130.0")), \
             patch("scarlight_cli.codex_runtime_plugin_migration.migrate",
