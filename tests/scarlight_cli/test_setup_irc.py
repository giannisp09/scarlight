"""Tests for IRC gateway configuration via `scarlight setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Scarlight install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Scarlight install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#scarlight")
            monkeypatch.setenv("IRC_NICKNAME", "scarlight-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `scarlight setup gateway` experience with IRC present."""
