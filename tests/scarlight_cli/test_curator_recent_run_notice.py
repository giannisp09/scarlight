...
The notice prints the most recent curator run summary on `scarlight update`,
...
session start) so users normally never see the rename map. `scarlight update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(scarlight_main)
