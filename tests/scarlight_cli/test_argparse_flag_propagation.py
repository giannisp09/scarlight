...
subparser, placing the flag BEFORE the subcommand (e.g. 'scarlight --yolo chat')
...
def _build_parser():
    """Build the scarlight argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets SCARLIGHT_YOLO_MODE regardless of flag position.
    ...
    def test_no_yolo_no_env(self):
        ...
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("SCARLIGHT_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `scarlight gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `scarlight <argv>` must exit 0 (help) rather than
