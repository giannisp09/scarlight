"""Tests for the top-level `./scarlight` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./scarlight` should use `scarlight_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "scarlight"
    called = []

    fake_main_module = types.ModuleType("scarlight_cli.main")

    def fake_main():
        called.append("scarlight_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "scarlight_cli.main", fake_main_module)
    ...
    assert called == ["scarlight_cli.main"]
