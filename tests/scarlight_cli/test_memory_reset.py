"""Tests for the `scarlight memory reset` CLI command.
...
- Profile-scoped reset (uses SCARLIGHT_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SCARLIGHT_HOME with memory files."""
    scarlight_home = tmp_path / ".scarlight"
    memories = scarlight_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nScarlight repo is at ~/.scarlight/scarlight-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
    ...
    return scarlight_home, memories
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `scarlight memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        scarlight_home = tmp_path / ".scarlight"
        (scarlight_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's SCARLIGHT_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        scarlight_home = tmp_path / ".scarlight"
        scarlight_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))

        # The memories dir won't exist; get_scarlight_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
