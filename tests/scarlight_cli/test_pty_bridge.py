"""Unit tests for scarlight_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf scarlight-ok"])
        try:
            output = _read_until(bridge, b"scarlight-ok")
            assert b"scarlight-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$SCARLIGHT_PTY_TEST\""],
            env={**os.environ, "SCARLIGHT_PTY_TEST": "pty-env-works"},
        )
        try:
