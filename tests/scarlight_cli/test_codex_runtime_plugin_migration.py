...
from scarlight_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MigrationReport,
    _format_toml_value,
...
# ---- end-to-end migrate(, expose_scarlight_tools=False) ----

class TestMigrate:
    def test_no_servers_no_plugins_no_perms_writes_placeholder(self, tmp_path):
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None, expose_scarlight_tools=False)
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_scarlight_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from scarlight_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from scarlight_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_scarlight_tools=False)
        assert report.written
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_scarlight_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_scarlight_tools=False)
        assert called["yes"] is False
    ...
    def test_expose_scarlight_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_scarlight_tools=True (production default), an
        [mcp_servers.scarlight-tools] entry is written so codex calls back
        into Scarlight for browser/web/delegate_task/vision/memory tools.

        This is the fix for 'all other tools that codex doesn't provide
        should be useable by scarlight' — quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_scarlight_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.scarlight-tools]" in text
        assert "scarlight_tools_mcp_server" in text
        ...
        assert "scarlight-tools" in report.migrated

    def test_expose_scarlight_tools_disabled_skips_entry(self, tmp_path):
        """expose_scarlight_tools=False suppresses the callback registration."""
        migrate({}, codex_home=tmp_path,
                discover_plugins=False,
                default_permission_profile=None,
                expose_scarlight_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.scarlight-tools]" not in text
        assert "scarlight_tools_mcp_server" not in text

    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_scarlight_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(scarlight_cfg, codex_home=tmp_path, expose_scarlight_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_scarlight_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_scarlight_tools=False)
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Scarlight' managed block, re-running
        ...
        migrate({"mcp_servers": {"scarlight-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.scarlight-mcp]" in final
    ...
    def test_skipped_keys_reported(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_scarlight_tools=False)
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_scarlight_tools=False)
