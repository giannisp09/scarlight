...
class TestSlackNativeSlashes:
    ...
    def test_scarlight_catchall_is_first(self):
        """``/scarlight`` must be reserved as the first slot so the legacy
        ``/scarlight <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "scarlight"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /scarlight <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `scarlight slack manifest`)."""
...
from scarlight_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch scarlight_cli.plugins.get_plugin_commands() to a fixed dict."""
        from scarlight_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/scarlight metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
