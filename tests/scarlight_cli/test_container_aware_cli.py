...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SCARLIGHT_HOME with .container-mode file."""
    scarlight_home = tmp_path / ".scarlight"
    scarlight_home.mkdir()
    monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))
    monkeypatch.delenv("SCARLIGHT_DEV", raising=False)

    container_mode = scarlight_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=scarlight-agent\n"
        "exec_user=scarlight\n"
        "scarlight_bin=/data/current-package/bin/scarlight\n"
    )
    return scarlight_home


def test_get_container_exec_info_returns_metadata(container_env):
    """Reads .container-mode and returns all fields including exec_user."""
    with patch("scarlight_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "scarlight-agent"
    assert info["exec_user"] == "scarlight"
    assert info["scarlight_bin"] == "/data/current-package/bin/scarlight"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("scarlight_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_scarlight_dev(container_env, monkeypatch):
    """Returns None when SCARLIGHT_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("SCARLIGHT_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_scarlight_dev_zero(container_env, monkeypatch):
    """SCARLIGHT_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("SCARLIGHT_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        scarlight_home = Path(tmpdir) / ".scarlight"
        scarlight_home.mkdir()
        (scarlight_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("scarlight_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_scarlight_home": lambda: scarlight_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SCARLIGHT_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=scarlight-custom\n"
        "exec_user=myuser\n"
        "scarlight_bin=/opt/scarlight/bin/scarlight\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "scarlight-custom"
    assert info["exec_user"] == "myuser"
    assert info["scarlight_bin"] == "/opt/scarlight/bin/scarlight"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from scarlight_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "scarlight"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "scarlight-agent" in cmd
    assert "/data/current-package/bin/scarlight" in cmd
    assert "chat" in cmd
