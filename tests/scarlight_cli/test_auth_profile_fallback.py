...
When ``SCARLIGHT_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.scarlight/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.scarlight            (has its own auth.json fixture)
    * Profile     -> tmp_path/.scarlight/profiles/coder   (active, SCARLIGHT_HOME points here)
    ...
    global_root = tmp_path / ".scarlight"
    global_root.mkdir()
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SCARLIGHT_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.scarlight as the "real home" — which is NOT equal
    # to the SCARLIGHT_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    scarlight_home = tmp_path / "classic"
    scarlight_home.mkdir()
    monkeypatch.setenv("SCARLIGHT_HOME", str(scarlight_home))

    _write(scarlight_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from scarlight_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: SCARLIGHT_HOME is set to a custom path that is NOT under
    # ~/.scarlight/profiles/ — get_default_scarlight_root() returns SCARLIGHT_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from scarlight_cli.auth import read_credential_pool, write_credential_pool
