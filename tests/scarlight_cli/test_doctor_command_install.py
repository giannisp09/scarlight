"""Tests for the Command Installation check in scarlight doctor."""
...
import scarlight_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal SCARLIGHT_HOME + PROJECT_ROOT for doctor tests."""
    ...
    scarlight_bin = venv_bin_dir / "scarlight"
    scarlight_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    scarlight_bin.chmod(0o755)
    ...
    return home, project, scarlight_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, scarlight_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "scarlight"
        cmd_link.symlink_to(scarlight_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "scarlight doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "scarlight"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == scarlight_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "scarlight"
        wrong_target = tmp_path / "wrong_scarlight"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        scarlight_bin = project / ".venv" / "bin" / "scarlight"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "scarlight"
        cmd_link.symlink_to(scarlight_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/scarlight" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/scarlight is a regular file (not symlink), accept it."""
        home, project, scarlight_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "scarlight"
        cmd_link.write_text("#!/bin/sh\nexec python -m scarlight_cli.main \"$@\"\n")
