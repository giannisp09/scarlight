...
``scarlight_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``scarlight --help``, ``scarlight version``,
``scarlight logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``scarlight --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from scarlight_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["scarlight", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["scarlight", "-w", "--tui"], None),
        (["scarlight", "version"], "version"),
        (["scarlight", "--tui", "chat"], "chat"),
        (["scarlight", "-w", "logs"], "logs"),
        (["scarlight", "chat", "hello world"], "chat"),
        (["scarlight", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["scarlight", "-m", "gpt5", "chat"], "chat"),
        (["scarlight", "--model", "gpt5", "chat", "hi"], "chat"),
        (["scarlight", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["scarlight", "-z", "hello world"], None),
        (["scarlight", "-z", "hello", "chat"], "chat"),
        (["scarlight", "--model=gpt5", "chat"], "chat"),     # inline form
        (["scarlight", "--", "chat"], "chat"),               # -- terminator
        (["scarlight", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["scarlight", "some-plugin-cmd"], "some-plugin-cmd"),
        (["scarlight", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["scarlight"],                          # bare → chat
        ["scarlight", "--help"],                # top-level help
        ["scarlight", "-h"],
        ["scarlight", "version"],               # known built-in
        ["scarlight", "logs"],
        ["scarlight", "gateway", "run"],
        ["scarlight", "--tui"],
        ["scarlight", "-w", "--tui"],
        ["scarlight", "chat", "hi"],
        ["scarlight", "help"],                  # accepted built-in-ish
        ["scarlight", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["scarlight", "meet", "join"],          # potential google_meet plugin
        ["scarlight", "honcho", "status"],      # potential memory plugin
        ["scarlight", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``scarlight help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"scarlight_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
