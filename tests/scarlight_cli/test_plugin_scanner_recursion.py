...
from scarlight_cli.plugins import PluginManager, PluginManifest
...
def _enable(scarlight_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<scarlight_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        user_plugins = scarlight_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(scarlight_home, "image_gen/openai")
    ...
    def test_flat_plugin_key_matches_name(self, tmp_path, monkeypatch):
        ...
        _enable(scarlight_home, "my-plugin")
    ...
    def test_category_dir_with_manifest_is_leaf(self, tmp_path, monkeypatch):
        ...
        # at the parent.
        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(scarlight_home, "image_gen")
        _enable(scarlight_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        scarlight_home = Path(os.environ["SCARLIGHT_HOME"])  # set by hermetic conftest fixture
        _write_plugin(scarlight_home / "plugins", ["p1"])
        _enable(scarlight_home, "p1")
...
class TestBackendGate:
    ...
    def test_user_backend_loads_when_enabled(self, tmp_path, monkeypatch):
        ...
        _enable(scarlight_home, "image_gen/fancy")
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(scarlight_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(scarlight_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(scarlight_home, "bad-img-plugin")
