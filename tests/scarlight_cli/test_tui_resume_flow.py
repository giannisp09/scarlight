...
def main_mod(monkeypatch):
    import scarlight_cli.main as mod
...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["scarlight", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["scarlight", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "scarlight_cli.plugins",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from scarlight_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "scarlight -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """scarlight -z bypasses ScarlightCLI, but recall still needs SessionDB."""
    from scarlight_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "scarlight_state", mod("scarlight_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "scarlight_cli.config",
        mod("scarlight_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "scarlight_cli.models",
        mod("scarlight_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "scarlight_cli.runtime_provider",
        mod(
            "scarlight_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "scarlight_cli.tools_config",
        mod("scarlight_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["SCARLIGHT_TUI_ACTIVE_SESSION_FILE"])
    ...
    active_path = Path(env["SCARLIGHT_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("scarlight-tui-active-session-")
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import scarlight_cli.main as main_mod
    ...
    monkeypatch.setitem(
        sys.modules, "scarlight_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "scarlight --tui --resume 20260409_000001_abc123" in out
    assert 'scarlight --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "scarlight --tui --resume actual_session" in out
