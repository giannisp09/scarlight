"""Tests for the kanban CLI surface (scarlight_cli.kanban)."""
...
from scarlight_cli import kanban as kc
...
def test_kanban_in_autocomplete_table():
    from scarlight_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_kanban_not_gateway_only():
    ...
    from scarlight_cli.commands import COMMAND_REGISTRY
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("SCARLIGHT_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("SCARLIGHT_KANBAN_BOARD") == "beta"
