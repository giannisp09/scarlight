...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.scarlight/.update_response."""
        adapter = self._make_adapter()
        scarlight_home = tmp_path / "scarlight_home"
        scarlight_home.mkdir()
        monkeypatch.setattr(
            "scarlight_constants.get_scarlight_home",
            lambda: scarlight_home,
        )
        ...
        response = scarlight_home / ".update_response"
