"""Tests for scarlight_constants module."""
...
from scarlight_constants import (
    ...
    get_default_scarlight_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultScarlightRoot:
    """Tests for get_default_scarlight_root() — Docker/custom deployment awareness."""

    def test_no_scarlight_home_returns_native(self, tmp_path, monkeypatch):
        """When SCARLIGHT_HOME is not set, returns ~/.scarlight."""
        monkeypatch.delenv("SCARLIGHT_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_scarlight_root() == tmp_path / ".scarlight"

    def test_scarlight_home_is_native(self, tmp_path, monkeypatch):
        """When SCARLIGHT_HOME = ~/.scarlight, returns ~/.scarlight."""
        native = tmp_path / ".scarlight"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SCARLIGHT_HOME", str(native))
        assert get_default_scarlight_root() == native

    def test_scarlight_home_is_profile(self, tmp_path, monkeypatch):
        """When SCARLIGHT_HOME is a profile under ~/.scarlight, returns ~/.scarlight."""
        native = tmp_path / ".scarlight"
    ...
    def test_scarlight_home_is_docker(self, tmp_path, monkeypatch):
        """When SCARLIGHT_HOME points outside ~/.scarlight (Docker), returns SCARLIGHT_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SCARLIGHT_HOME", str(docker_home))
        assert get_default_scarlight_root() == docker_home

    def test_scarlight_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any SCARLIGHT_HOME outside ~/.scarlight is treated as the root."""
        custom = tmp_path / "my-scarlight-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SCARLIGHT_HOME", str(custom))
        assert get_default_scarlight_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (SCARLIGHT_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SCARLIGHT_HOME", str(profile))
        assert get_default_scarlight_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(scarlight_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(scarlight_constants, "_container_detected", True)
        assert is_container() is True
