...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    scarlight_state = ModuleType("scarlight_state")
    scarlight_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "scarlight_state", scarlight_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Scarlight"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Scarlight"
