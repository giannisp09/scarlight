"""Tests for scarlight_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Scarlight entry point
(scarlight, scarlight-agent, scarlight-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Scarlight entry point imports scarlight_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported scarlight_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("scarlight_bootstrap", None)
    import scarlight_bootstrap  # noqa: WPS433
    return scarlight_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Scarlight CLI
...
class TestEntryPointsImportBootstrap:
    """Every Scarlight entry point must import scarlight_bootstrap as its
    ...
    # Entry points that invoke Scarlight as a process.  Each one must
    # import scarlight_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "scarlight_cli/main.py",   # scarlight CLI (console_script)
        "run_agent.py",          # scarlight-agent (console_script)
        "acp_adapter/entry.py",  # scarlight-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import scarlight_bootstrap' and that
        ...
        half-finished ``scarlight update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``scarlight_bootstrap`` as a top-level module) leaves scarlight
        ...
        """
        # Resolve relative to the scarlight-agent repo root.  Tests live
        # at tests/test_scarlight_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # scarlight start even when scarlight_bootstrap hasn't been
        ...
        assert first_import_name == "scarlight_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'scarlight_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import scarlight_bootstrap' line to be the first import."
        )
