...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Scarlight starts and finds ``~/.openclaw/``.

    Points users at ``scarlight claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``scarlight claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Scarlight, run "
        "`scarlight claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `scarlight claw cleanup` (renames it to ~/.openclaw.pre-migration — "
