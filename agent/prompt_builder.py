...
from scarlight_constants import get_scarlight_home, get_skills_dir, is_wsl
...
_SCARLIGHT_MD_NAMES = (".scarlight.md", "SCARLIGHT.md")


def _find_scarlight_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.scarlight.md`` or ``SCARLIGHT.md``.
    ...
    for directory in [current, *current.parents]:
        for name in _SCARLIGHT_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Scarlight Agent, an intelligent AI assistant created by Nous Research. "
...
SCARLIGHT_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Scarlight Agent "
    "itself, load the `scarlight-agent` skill with skill_view(name='scarlight-agent') "
    "before answering. Docs: https://scarlight-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.scarlight/kanban.db`. Your task id is in "
    "`$SCARLIGHT_KANBAN_TASK`; your workspace is `$SCARLIGHT_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SCARLIGHT_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `scarlight kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Scarlight WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Scarlight itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Scarlight runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Scarlight itself is running. The host OS, home, and cwd "
                f"of the Scarlight process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Scarlight "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_scarlight_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.scarlight/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("SCARLIGHT_PLATFORM")
        or get_session_env("SCARLIGHT_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Scarlight Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `scarlight-agent` skill "
            "first. It has the actual commands (e.g. `scarlight config set …`, `scarlight tools`, "
            "`scarlight setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: scarlight setup, scarlight setup tools, scarlight setup terminal, scarlight status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from SCARLIGHT_HOME and return its content, or None.
    ...
    """
    try:
        from scarlight_cli.config import ensure_scarlight_home
        ensure_scarlight_home()
    except Exception as e:
        logger.debug("Could not ensure SCARLIGHT_HOME before loading SOUL.md: %s", e)

    soul_path = get_scarlight_home() / "SOUL.md"
...
def _load_scarlight_md(cwd_path: Path) -> str:
    """.scarlight.md / SCARLIGHT.md — walk to git root."""
    scarlight_md_path = _find_scarlight_md(cwd_path)
    if not scarlight_md_path:
        return ""
    try:
        content = scarlight_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = scarlight_md_path.name
        try:
            rel = str(scarlight_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".scarlight.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", scarlight_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .scarlight.md / SCARLIGHT.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SCARLIGHT_HOME is independent and always included when present.
    ...
    # SOUL.md from SCARLIGHT_HOME only — skip when already loaded as identity
    if not skip_soul:
