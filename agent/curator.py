...
def _state_file() -> Path:
    return get_scarlight_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.scarlight/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``scarlight update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``scarlight curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `scarlight curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Scarlight' background skill CURATOR. This is an "
    ...
    "into ~/.scarlight/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: scarlight-config-*, scarlight-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.scarlight/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.scarlight/logs/curator/``)
    ...
    authored skill data in ``~/.scarlight/skills/``.

    ``ensure_scarlight_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_scarlight_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every scarlight install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: scarlight curator status
        keep an umbrella stable: scarlight curator pin document-tools
    ...
    lines.append("full report: scarlight curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: scarlight curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.scarlight/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.scarlight/skills/.archive/` for "
            "safety and can be restored via `scarlight curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.scarlight/skills/.archive/`. "
            "Restore any via `scarlight curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `scarlight curator restore <name>`")
    lines.append("- All archives live under `~/.scarlight/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `scarlight curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `scarlight curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``scarlight model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `scarlight model` → auxiliary
