...
as ``kind: backend``) or ``~/.scarlight/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$SCARLIGHT_HOME/cache/images/``, creating parents as needed."""
    from scarlight_constants import get_scarlight_home

    path = get_scarlight_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$SCARLIGHT_HOME/cache/images/``.
