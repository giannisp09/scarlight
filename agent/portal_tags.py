...
Every Scarlight request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Scarlight Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=scarlight-agent",
        "client=scarlight-client-v<__version__>",
    ]

The version is sourced live from ``scarlight_cli.__version__`` so it auto-aligns
...
``scarlight_cli.__version__`` is the canonical source of truth.
"""
...
def _scarlight_version() -> str:
    """Return the current Scarlight release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``scarlight_cli`` cannot be imported (should
...
def scarlight_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=scarlight-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=scarlight-client-v{_scarlight_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=scarlight-agent", scarlight_client_tag()]
