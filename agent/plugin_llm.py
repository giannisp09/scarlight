...
Plugins built on Scarlight Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Scarlight
...
:class:`~scarlight_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Scarlight' main config
...
handles every provider, fallback chain, and per-task override Scarlight
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`scarlight_cli.plugins.PluginContext`
