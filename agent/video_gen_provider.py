...
as ``kind: backend``) or ``~/.scarlight/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``scarlight tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``scarlight tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``scarlight tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``scarlight tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SCARLIGHT_HOME/cache/videos/``, creating parents as needed."""
    from scarlight_constants import get_scarlight_home

    path = get_scarlight_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SCARLIGHT_HOME/cache/videos/``.
