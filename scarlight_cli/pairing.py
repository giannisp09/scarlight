...
Usage:
    scarlight pairing list              # Show all pending + approved users
    scarlight pairing approve <platform> <code>  # Approve a pairing code
    scarlight pairing revoke <platform> <user_id> # Revoke user access
    scarlight pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle scarlight pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: scarlight pairing {list|approve|revoke|clear-pending}")
        print("Run 'scarlight pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.scarlight/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'scarlight pairing list' to see pending codes.\n")
