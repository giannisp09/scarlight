"""
Status command for scarlight CLI.

Shows the status of all Scarlight Agent components.
"""
...
from scarlight_cli.auth import AuthError, resolve_provider
from scarlight_cli.colors import Colors, color
from scarlight_cli.config import get_env_path, get_env_value, get_scarlight_home, load_config
from scarlight_cli.models import provider_label
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``scarlight config``'s
...
def show_status(args):
    """Show status of all Scarlight Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Scarlight Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: scarlight auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: scarlight model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: scarlight auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: scarlight model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from scarlight_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'scarlight-agent[vercel]')"
    ...
    try:
        from scarlight_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   scarlight gateway")
    ...
    jobs_file = get_scarlight_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_scarlight_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'scarlight doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'scarlight setup' to configure", Colors.DIM))
    print()
