...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or scarlight -c.",
    ...
    "Ctrl+Z suspends Scarlight to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "scarlight -c resumes your most recent CLI session. scarlight -c \"project name\" resumes by title.",
    "scarlight -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "scarlight -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "scarlight chat -t web,terminal enables only specific toolsets for a focused session.",
    "scarlight chat -s github-pr-workflow preloads a skill at launch.",
    "scarlight chat -q \"query\" runs a single non-interactive query and exits.",
    "scarlight chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "scarlight chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "scarlight --yolo bypasses all dangerous command approval prompts for the entire session.",
    "scarlight chat --source telegram tags the session for filtering in scarlight sessions list.",
    "scarlight -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "scarlight doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "scarlight dump outputs a compact setup summary — great for bug reports.",
    "scarlight config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "scarlight config edit opens config.yaml in your default editor.",
    "scarlight config check scans for missing or stale configuration options.",
    "scarlight sessions browse opens an interactive session picker with search.",
    "scarlight sessions stats shows session counts by platform and database size.",
    "scarlight sessions prune --older-than 30 cleans up old sessions.",
    "scarlight skills search react --source skills-sh searches the skills.sh public directory.",
    "scarlight skills check scans installed hub skills for upstream updates.",
    "scarlight skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "scarlight skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "scarlight mcp add github --command npx adds MCP servers from the command line.",
    "scarlight mcp serve runs Scarlight itself as an MCP server for other agents.",
    "scarlight auth add lets you add multiple API keys for credential pool rotation.",
    "scarlight completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "scarlight logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "scarlight backup creates a zip backup of your entire Scarlight home directory.",
    "scarlight profile create coder creates an isolated profile that becomes its own command.",
    "scarlight profile create work --clone copies your current config and keys to a new profile.",
    "scarlight update syncs new bundled skills to ALL profiles automatically.",
    "scarlight gateway install sets up Scarlight as a system service (systemd/launchd).",
    "scarlight memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as scarlight -w).",
    ...
    "SCARLIGHT_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Scarlight tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'scarlight profile create coder' creates the 'coder' command.",
    "scarlight profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Scarlight prints a resume command with session ID and stats.",
    "scarlight sessions export backup.jsonl exports all sessions for backup or analysis.",
    "scarlight -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "scarlight skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .scarlight/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: scarlight cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Scarlight runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "scarlight gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Scarlight via a local macOS server.",
    ...
    "Context files (.scarlight.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.scarlight/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Scarlight loads project context from .scarlight.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — scarlight tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.scarlight/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "scarlight chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "scarlight plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "scarlight acp runs Scarlight as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "SCARLIGHT_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "scarlight login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "scarlight status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.scarlight/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Scarlight your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.scarlight/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Scarlight auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.scarlight/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at SCARLIGHT_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "SCARLIGHT_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SCARLIGHT_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.scarlight/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'scarlight profile create ls' would be rejected.",
    "scarlight profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "scarlight claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SCARLIGHT_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via scarlight tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.scarlight/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.scarlight/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Scarlight auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Scarlight config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Scarlight is working.',
    ...
    '/reload reloads ~/.scarlight/.env into the running session — pick up new API keys without restarting.',
    ...
    "SCARLIGHT_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.scarlight/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.scarlight/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'scarlight curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "scarlight curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'scarlight curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'scarlight auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'SCARLIGHT_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "SCARLIGHT_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'scarlight dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'scarlight dashboard --tui embeds the full Scarlight TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.scarlight/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.scarlight/dashboard-plugins/ — no npm build required.',
    ...
    'SCARLIGHT_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'SCARLIGHT_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'SCARLIGHT_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'SCARLIGHT_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.scarlight/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.scarlight/skills/ so drop-in skills appear without restarting the session.',
    ...
    'scarlight -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'scarlight chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'scarlight chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'scarlight chat --ignore-user-config skips ~/.scarlight/config.yaml — reproducible bug reports and CI runs.',
    "scarlight chat --source tool tags programmatic chats so they don't clutter scarlight sessions list.",
    'scarlight dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'scarlight sessions rename <ID> "new title" renames any past session; scarlight sessions delete <ID> removes one.',
    'scarlight import restores a session export or profile archive produced by sessions export or profile export.',
    'scarlight fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'scarlight pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'scarlight setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'scarlight status --deep runs the full health sweep across every component; plain scarlight status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'SCARLIGHT_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SCARLIGHT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.scarlight/plugins/ — trust-gated by design.',
    "SCARLIGHT_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SCARLIGHT_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SCARLIGHT_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SCARLIGHT_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SCARLIGHT_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SCARLIGHT_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'SCARLIGHT_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'SCARLIGHT_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SCARLIGHT_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SCARLIGHT_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SCARLIGHT_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Scarlight block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from scarlight sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.scarlight/dashboard-plugins/.',
]
