"""`scarlight checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.scarlight/checkpoints/``.  Actions:

    scarlight checkpoints               # same as `status`
    scarlight checkpoints status        # total size, project count, breakdown
    scarlight checkpoints list          # per-project checkpoint counts + workdir
    scarlight checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    scarlight checkpoints clear [-f]    # nuke the entire base (asks first)
    scarlight checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    scarlight checkpoints
    scarlight checkpoints prune --retention-days 3 --max-size-mb 200
    scarlight checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: scarlight checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``scarlight checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `scarlight checkpoints` → status
