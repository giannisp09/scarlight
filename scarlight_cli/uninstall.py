"""
Scarlight Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.scarlight/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Scarlight PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing scarlight-agent or scarlight PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Scarlight Agent" comment and following line
                if '# Scarlight Agent' in line or '# scarlight-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('scarlight' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing scarlight
                if 'scarlight' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the scarlight wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "scarlight",
        Path("/usr/local/bin/scarlight"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains scarlight_cli reference)
                content = wrapper.read_text()
                if 'scarlight_cli' in content or 'scarlight-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``scarlight gateway run`` processes
    ...
    try:
        from scarlight_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from scarlight_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SCARLIGHT_HOME`` and ``SCARLIGHT_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\scarlight\git\cmd``, ``%LOCALAPPDATA%\scarlight\git\bin``,
#      ``%LOCALAPPDATA%\scarlight\git\usr\bin``, ``%LOCALAPPDATA%\scarlight\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\scarlight\git\`` and Node to
#      ``%LOCALAPPDATA%\scarlight\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``scarlight dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\scarlight\gateway-service\`` and sometimes
...
def _scarlight_path_markers(scarlight_home: Path) -> list[str]:
    """Path-entry substrings that identify Scarlight-owned User-PATH entries."""
    root = str(scarlight_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare scarlight-agent install dir.
    markers = [root + "\\scarlight-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SCARLIGHT_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "scarlight".  We don't want to catch
    # unrelated entries like "cscarlight-foo" or "ephermeral", so we look for
    # backslash-scarlight as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(scarlight_home: Path) -> list[str]:
    """Strip Scarlight-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _scarlight_path_markers(scarlight_home)
            kept: list[str] = []
...
def remove_scarlight_env_vars_windows() -> list[str]:
    """Delete SCARLIGHT_HOME and SCARLIGHT_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SCARLIGHT_HOME", "SCARLIGHT_GIT_BASH_PATH"):
                try:
...
def _is_default_scarlight_home(scarlight_home: Path) -> bool:
    """Return True when ``scarlight_home`` points at the default (non-profile) root."""
    try:
        from scarlight_constants import get_default_scarlight_root
        return scarlight_home.resolve() == get_default_scarlight_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SCARLIGHT_HOME directory.

    We shell out to ``scarlight -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SCARLIGHT_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m scarlight_cli.main` so we don't depend on a `scarlight`
    #    wrapper that may be half-removed mid-uninstall.
    scarlight_invocation = [_sys.executable, "-m", "scarlight_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                scarlight_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's SCARLIGHT_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.scarlight/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.scarlight/ for future reinstall
    """
    project_root = get_project_root()
    scarlight_home = get_scarlight_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie SCARLIGHT_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_scarlight_home(scarlight_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Scarlight Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {scarlight_home / 'config.yaml'}")
    print(f"  Secrets: {scarlight_home / '.env'}")
    print(f"  Data:    {scarlight_home / 'cron/'}, {scarlight_home / 'sessions/'}, {scarlight_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SCARLIGHT_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Scarlight data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Scarlight code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in scarlight_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\scarlight\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(scarlight_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Scarlight-owned PATH entries in User environment")

        log_info("Removing SCARLIGHT_HOME / SCARLIGHT_GIT_BASH_PATH User env vars...")
        removed_env = remove_scarlight_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Scarlight-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing scarlight command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.scarlight/, just remove the scarlight-agent subdir
            if scarlight_home in project_root.parents or project_root.parent == scarlight_home:
    ...
    #     under SCARLIGHT_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(scarlight_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(scarlight_home)
    ...
    # 5. Optionally remove ~/.scarlight/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SCARLIGHT_HOME dirs live under
        ...
        try:
            if scarlight_home.exists():
                shutil.rmtree(scarlight_home)
                log_success(f"Removed {scarlight_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {scarlight_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {scarlight_home}")
    ...
    print()
    print("Thank you for using Scarlight Agent! ⚕")
    print()
