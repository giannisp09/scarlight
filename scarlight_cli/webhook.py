"""scarlight webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    scarlight webhook subscribe <name> [options]
    scarlight webhook list
    scarlight webhook remove <name>
    scarlight webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.scarlight/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
    _dhh = display_scarlight_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     scarlight gateway setup
  ...
  Then start the gateway: scarlight gateway run
"""
...
def webhook_command(args):
    """Entry point for 'scarlight webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: scarlight webhook {subscribe|list|remove|test}")
        print("Run 'scarlight webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (scarlight gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: scarlight webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from scarlight webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (scarlight gateway run)")
