...
on Japanese locales, etc.).  Scarlight's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``scarlight_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Scarlight is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SCARLIGHT_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SCARLIGHT_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Scarlight-managed Git install directories so
    ...
    # already-running shells don't see the change — which means scarlight
    ...
    # install without Scarlight-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Scarlight's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Scarlight's
    ``scarlight config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Scarlight (User env var in Windows Settings, or
    export in a PowerShell profile) and Scarlight picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Scarlight-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\scarlight\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``scarlight`` from right after install) retain their old PATH.

    Any subprocess Scarlight spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Scarlight-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "scarlight", "git", "cmd"),
        os.path.join(local_appdata, "scarlight", "git", "bin"),
        os.path.join(local_appdata, "scarlight", "git", "usr", "bin"),
        # Scarlight venv Scripts directory — host of the scarlight.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes scarlight, but harmless to include.
        os.path.join(local_appdata, "scarlight", "scarlight-agent", "venv", "Scripts"),
