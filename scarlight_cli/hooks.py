"""scarlight hooks — inspect and manage shell-script hooks.

Usage::

    scarlight hooks list
    scarlight hooks test <event> [--for-tool X] [--payload-file F]
    scarlight hooks revoke <command>
    scarlight hooks doctor

Consent records live under ``~/.scarlight/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.scarlight/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``scarlight hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: scarlight hooks {list|test|revoke|doctor}")
        print("Run 'scarlight hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.scarlight/config.yaml.")
        print("See `scarlight hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `scarlight hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `scarlight hooks test` and `scarlight hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Scarlight wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `scarlight hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `scarlight hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `scarlight hooks doctor`.")
