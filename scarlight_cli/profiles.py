"""
Profile management for multiple isolated Scarlight instances.

Each profile is a fully independent SCARLIGHT_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.scarlight/profiles/<name>/`` by default.

The "default" profile is ``~/.scarlight`` itself — backward compatible,
zero migration needed.

Usage::

    scarlight profile create coder          # fresh profile + bundled skills
    scarlight profile create coder --clone  # also copy config, .env, SOUL.md, skills
    scarlight profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    scarlight -p coder chat                 # or via flag
    scarlight profile use coder             # set as sticky default
    scarlight profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See scarlight_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.scarlight``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   scarlight-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `scarlight profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `scarlight
...
# `scarlight skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Scarlight infrastructure directories that only the default profile
         (``~/.scarlight``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_scarlight_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.scarlight) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "scarlight-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "scarlight", "default", "test", "tmp", "root", "sudo",
})

# Scarlight subcommands that cannot be used as profile names/aliases
_SCARLIGHT_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the scarlight root, NOT to the current SCARLIGHT_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SCARLIGHT_HOME points outside
    ``~/.scarlight``, profiles live under ``SCARLIGHT_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_scarlight_home() / "profiles"


def _get_default_scarlight_home() -> Path:
    """Return the default (pre-profile) SCARLIGHT_HOME path.

    In standard deployments this is ``~/.scarlight``.
    In Docker/custom deployments where SCARLIGHT_HOME is outside ``~/.scarlight``
    (e.g. ``/opt/data``), returns SCARLIGHT_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_scarlight_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Scarlight installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SCARLIGHT_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_scarlight_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, scarlight subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _SCARLIGHT_SUBCOMMANDS:
        return f"'{canon}' conflicts with a scarlight subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``scarlight profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_scarlight_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``scarlight update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.scarlight)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.scarlight/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `scarlight update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`scarlight profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `scarlight update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SCARLIGHT_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``scarlight profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SCARLIGHT_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.scarlight).\n"
            "To remove everything, use: scarlight uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SCARLIGHT_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SCARLIGHT_HOME")
    try:
        os.environ["SCARLIGHT_HOME"] = str(profile_dir)
        from scarlight_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SCARLIGHT_HOME"] = old_home
        elif "SCARLIGHT_HOME" in os.environ:
            del os.environ["SCARLIGHT_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.scarlight/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: scarlight profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SCARLIGHT_HOME.

    Returns ``"default"`` if SCARLIGHT_HOME is not set or points to ``~/.scarlight``.
    Returns the profile name if SCARLIGHT_HOME points into ``~/.scarlight/profiles/<name>``.
    Returns ``"custom"`` if SCARLIGHT_HOME is set to an unrecognized path.
    """
    from scarlight_constants import get_scarlight_home
    scarlight_home = get_scarlight_home()
    resolved = scarlight_home.resolve()

    default_resolved = _get_default_scarlight_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.scarlight itself — its parent is ~/ and its
        # directory name is ".scarlight", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: scarlight profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.scarlight itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.scarlight). "
            "Specify a different name: scarlight profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="scarlight_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"scarlight.{old_name}"
    new_host = f"scarlight.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_scarlight_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SCARLIGHT_HOME path string.

    Called early in the CLI entry point, before any scarlight modules
    are imported, to set the SCARLIGHT_HOME environment variable.
    """
