"""Profile distributions — shareable, packaged Scarlight profiles via git.

A distribution is a Scarlight profile published as a git repository (or
...
* ``scarlight profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``scarlight skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``scarlight profile``, not a parallel tree):

    scarlight profile install <source> [--name N] [--alias] [--force] [--yes]
    scarlight profile update  <name>  [--force-config] [--yes]
    scarlight profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "scarlight_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".scarlight_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "scarlight-agent", ".worktrees", "profiles", "bin", "node_modules",
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Scarlight {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Scarlight distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``scarlight profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Scarlight profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Scarlight distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.scarlight).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="scarlight_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `scarlight profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`scarlight profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="scarlight_dist_update_") as tmp:
        plan = plan_install(
