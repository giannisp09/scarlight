"""``scarlight logs`` — view and filter Scarlight log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.scarlight/logs/``.

Usage examples::

    scarlight logs                    # last 50 lines of agent.log
    scarlight logs -f                 # follow agent.log in real time
    scarlight logs errors             # last 50 lines of errors.log
    scarlight logs gateway -n 100    # last 100 lines of gateway.log
    scarlight logs --level WARNING    # only WARNING+ lines
    scarlight logs --session abc123   # filter by session ID substring
    scarlight logs --component tools  # only tool-related lines
    scarlight logs --since 1h         # lines from the last hour
    scarlight logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_scarlight_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Scarlight runs — try 'scarlight chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_scarlight_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_scarlight_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_scarlight_home()}/logs/")
        return

    print(f"Log files in {display_scarlight_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'scarlight chat' to generate logs)")
