"""
Configuration management for Scarlight Agent.

Config files are stored in ~/.scarlight/ for easy access:
- ~/.scarlight/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.scarlight/.env         - API keys and secrets

This module provides:
- scarlight config          - Show current configuration
- scarlight config edit     - Open config in editor
- scarlight config set      - Set a specific value
- scarlight config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.scarlight/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``scarlight logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  scarlight config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `scarlight plugins enable
    # observability/langfuse` or `scarlight tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SCARLIGHT_LANGFUSE_ENV",
    "SCARLIGHT_LANGFUSE_RELEASE",
    "SCARLIGHT_LANGFUSE_SAMPLE_RATE",
    "SCARLIGHT_LANGFUSE_MAX_CHARS",
    "SCARLIGHT_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from scarlight_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SCARLIGHT_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_scarlight_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Scarlight is running in package-manager-managed mode.

    Two signals: the SCARLIGHT_MANAGED env var (set by the systemd service),
    or a .managed marker file in SCARLIGHT_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade scarlight-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "scarlight update"


def format_managed_message(action: str = "modify this Scarlight installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("SCARLIGHT_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Scarlight installation is managed by NixOS "
            f"(SCARLIGHT_MANAGED={env_hint}).\n"
            "Edit services.scarlight-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Scarlight installation is managed by Homebrew "
            f"(SCARLIGHT_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade scarlight-agent"
        )

    return (
        f"Cannot {action}: this Scarlight installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Scarlight."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SCARLIGHT_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, scarlight_bin
    or None if container mode is not active, we're already inside the
    container, or SCARLIGHT_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SCARLIGHT_DEV") == "1":
        return None
    ...
    container_mode_file = get_scarlight_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "scarlight-agent")
    exec_user = info.get("exec_user", "scarlight")
    scarlight_bin = info.get("scarlight_bin", "/data/current-package/bin/scarlight")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "scarlight_bin": scarlight_bin,
    }
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the scarlight group can
    share state with the gateway service.

    The mode can be overridden via the SCARLIGHT_HOME_MODE environment variable
    (e.g. SCARLIGHT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SCARLIGHT_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SCARLIGHT_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Scarlight runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SCARLIGHT_CONTAINER") or os.environ.get("SCARLIGHT_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SCARLIGHT_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SCARLIGHT_HOME if the user doesn't have one yet."""
...
def ensure_scarlight_home():
    """Ensure ~/.scarlight directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_scarlight_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_scarlight_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SCARLIGHT_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Scarlight-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Scarlight auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Scarlight sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Scarlight
        ...
        #  "/home/user/.scarlight/cache/documents:/output"]
        ...
        # bundled Scarlight image, which drops to the `scarlight` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Scarlight sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Scarlight should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``scarlight chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.scarlight/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: scarlight sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Scarlight truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``scarlight kanban specify`` (single id or --all). Set a
        ...
        # model; override via `scarlight model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `scarlight --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `scarlight -c` muscle memory.  Default off so existing
        # users aren't surprised.  SCARLIGHT_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/scarlight`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.scarlight/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.scarlight/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Scarlight feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Scarlight auto-pauses the goal and
    ...
    # always goes to ~/.scarlight/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${SCARLIGHT_SKILL_DIR} and ${SCARLIGHT_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `scarlight curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.scarlight/skills/ into
        # ~/.scarlight/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `scarlight curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for scarlight-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Scarlight Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (SCARLIGHT_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.scarlight/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SCARLIGHT_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `scarlight doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``scarlight_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Scarlight to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via SCARLIGHT_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `scarlight -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     scarlight-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.scarlight/logs/.
    ...
    # update model picker lists without shipping a scarlight-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://scarlight-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `scarlight model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.scarlight/state.db.
    ...
    "sessions": {
        ...
        # default of ``scarlight sessions prune``.
        "retention_days": 90,
    ...
    # ``scarlight update`` behaviour.
    "updates": {
        # Run a full ``scarlight backup``-style zip of SCARLIGHT_HOME before every
        # ``scarlight update``.  Backups land in ``<SCARLIGHT_HOME>/backups/`` and
        # can be restored with ``scarlight import <path>``.  Off by default —
        # on large SCARLIGHT_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<SCARLIGHT_HOME>/lsp/bin/`` on first use.
...
OPTIONAL_ENV_VARS = {
    ...
    "SCARLIGHT_GEMINI_CLIENT_SECRET": {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'scarlight model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Scarlight auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @scarlight:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SCARLIGHT_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #scarlight)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: scarlight-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'scarlight-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Scarlight API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Scarlight API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Scarlight API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SCARLIGHT_TOOL_PROGRESS and SCARLIGHT_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "SCARLIGHT_TOOL_PROGRESS": {
    ...
    "SCARLIGHT_PREFILL_MESSAGES_FILE": {
    ...
    "SCARLIGHT_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.scarlight.config`` entries, then checks
    ...
    try:
        ...
        # should never break `scarlight update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Scarlight versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``scarlight_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'scarlight setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Scarlight won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'scarlight doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SCARLIGHT_HOME", "~/.scarlight")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SCARLIGHT_TOOL_PROGRESS")
            old_mode = get_env_value("SCARLIGHT_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from SCARLIGHT_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from SCARLIGHT_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SCARLIGHT_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from SCARLIGHT_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SCARLIGHT_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_scarlight_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `scarlight plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `scarlight curator status` has no
    ...
    #   3. Creates `~/.scarlight/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_scarlight_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_scarlight_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: scarlight config set <key> <value>")
    ...
    # metadata.scarlight.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _scarlight_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.scarlight/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    ...
    (which change ``SCARLIGHT_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_scarlight_home()
        config_path = get_config_path()
        path_key = str(config_path)
...
#   openai-codex (OAuth — scarlight auth) — OpenAI Codex
#   nous         (OAuth — scarlight auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.scarlight/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.scarlight/.env.
    ...
    interactive menu render (`scarlight tools`, `scarlight setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `scarlight tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Scarlight env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.scarlight/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.scarlight/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.scarlight/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.scarlight/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Scarlight — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Scarlight vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.scarlight/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Scarlight Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  scarlight config edit     # Edit config file", Colors.DIM))
    print(color("  scarlight config set <key> <value>", Colors.DIM))
    print(color("  scarlight setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: scarlight config set <key> <value>")
            print()
            print("Examples:")
            print("  scarlight config set model anthropic/claude-sonnet-4")
            print("  scarlight config set terminal.backend docker")
            print("  scarlight config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'scarlight config migrate' to add them")
        ...
        print("  scarlight config           Show current configuration")
        print("  scarlight config edit      Open config in editor")
        print("  scarlight config set <key> <value>   Set a config value")
        print("  scarlight config check     Check for missing/outdated config")
        print("  scarlight config migrate   Update config with new options")
        print("  scarlight config path      Show config file path")
        print("  scarlight config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``scarlight config`` UI so users
