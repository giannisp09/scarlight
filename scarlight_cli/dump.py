"""
Dump command for scarlight CLI.

Outputs a compact, plain-text summary of the user's Scarlight setup
...
from scarlight_cli.config import get_scarlight_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``scarlight dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from scarlight_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(scarlight_home: Path) -> int:
...
def _cron_summary(scarlight_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = scarlight_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    load_scarlight_dotenv(
        scarlight_home=env_path.parent,
    ...
    lines.append(f"scarlight_home:      {display_scarlight_home()}")
    ...
    toolsets = config.get("toolsets", ["scarlight-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(scarlight_home)}")
    lines.append(f"  skills:             {_count_skills(scarlight_home)}")
