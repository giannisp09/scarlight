"""Migrate Scarlight' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Scarlight' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Scarlight mcp_servers.<n>.command/args/env  → codex stdio transport
  Scarlight mcp_servers.<n>.url/headers       → codex streamable_http transport
  Scarlight mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Scarlight mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Scarlight-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Scarlight' own
...
MIGRATION_MARKER = (
    "# managed by scarlight-agent — `scarlight codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end scarlight-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Scarlight config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Scarlight' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, scarlight_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Scarlight MCP server config to the codex inline-table dict
    ...
    if not isinstance(scarlight_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(scarlight_cfg.get("command"))
    has_url = bool(scarlight_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(scarlight_cfg["command"])
        args = scarlight_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = scarlight_cfg.get("env") or {}
        if env:
            # Codex expects string values
            out["env"] = {str(k): str(v) for k, v in env.items()}
        cwd = scarlight_cfg.get("cwd")
        if cwd:
            out["cwd"] = str(cwd)
    elif has_url:
        # streamable_http transport (codex covers both http and SSE here)
        out["url"] = str(scarlight_cfg["url"])
        headers = scarlight_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Scarlight' transport: sse hint is informational; codex auto-negotiates
        if scarlight_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in scarlight_cfg:
        try:
            out["tool_timeout_sec"] = float(scarlight_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in scarlight_cfg:
        try:
            out["startup_timeout_sec"] = float(scarlight_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if scarlight_cfg.get("enabled") is False:
        out["enabled"] = False

    # Detect keys we explicitly drop with warning
    for key in scarlight_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_SCARLIGHT_KEYS:
            skipped.append(f"{key} (unknown Scarlight key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (SCARLIGHT_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Scarlight)")
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Scarlight
    versions."""
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="scarlight-migration")
...
def _build_scarlight_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Scarlight' own
    ...
    so a scarlight installed under /opt/, /usr/local/, or a venv all work.
    SCARLIGHT_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # SCARLIGHT_HOME passes through if set so the MCP subprocess sees the
    # same config / auth / sessions DB as the parent CLI.
    scarlight_home = os.environ.get("SCARLIGHT_HOME")
    if scarlight_home:
        env["SCARLIGHT_HOME"] = scarlight_home
    # PYTHONPATH passes through so a worktree-launched scarlight finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["SCARLIGHT_QUIET"] = "1"
    env["SCARLIGHT_REDACT_SECRETS"] = env.get("SCARLIGHT_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.scarlight_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    scarlight_config: dict,
    *,
    codex_home: Optional[Path] = None,
    dry_run: bool = False,
    discover_plugins: bool = True,
    default_permission_profile: Optional[str] = ":workspace",
    expose_scarlight_tools: bool = True,
) -> MigrationReport:
    """Translate Scarlight mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        scarlight_config: full ~/.scarlight/config.yaml dict
        ...
        expose_scarlight_tools: when True (default), register Scarlight' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Scarlight for tools
    ...
    scarlight_servers = (scarlight_config or {}).get("mcp_servers") or {}
    if not isinstance(scarlight_servers, dict):
        report.errors.append(
            "mcp_servers in Scarlight config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in scarlight_servers.items():
    ...
    # Inject Scarlight' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Scarlight for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/scarlight_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_scarlight_tools:
        translated["scarlight-tools"] = _build_scarlight_tools_mcp_entry()
        if "scarlight-tools" not in report.migrated:
            report.migrated.append("scarlight-tools")
