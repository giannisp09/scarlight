"""Shell completion script generation for scarlight CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_scarlight_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Scarlight Agent bash completion
# Add to ~/.bashrc:
#   eval "$(scarlight completion bash)"

_scarlight_profiles() {{
    local profiles_dir="$HOME/.scarlight/profiles"
...
_scarlight_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_scarlight_profiles)" -- "$cur"))
        return
    fi
...
complete -F _scarlight_completion scarlight
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef scarlight
# Scarlight Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(scarlight completion zsh)"

_scarlight_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.scarlight/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.scarlight/profiles 2>/dev/null)}}")
    fi
...
_scarlight() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_scarlight_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'scarlight command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Scarlight Agent fish completion",
        "# Add to your config:",
        "#   scarlight completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __scarlight_profiles",
        "    echo default",
        "    if test -d $HOME/.scarlight/profiles",
        "        ls $HOME/.scarlight/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c scarlight -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c scarlight -f -s p -l profile"
        " -d 'Profile name' -xa '(__scarlight_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__scarlight_profiles)' -d 'Profile name'"
                )
