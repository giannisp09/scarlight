...
# Python-driven tool session (e.g. Scarlight terminal tool). A pre-set PYTHONPATH
...
SCARLIGHT_HOME="${SCARLIGHT_HOME:-$HOME/.scarlight}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${SCARLIGHT_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SCARLIGHT_INSTALL_DIR"
...
#   code at /usr/local/lib/scarlight-agent, command at /usr/local/bin/scarlight,
#   data still at /root/.scarlight (SCARLIGHT_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --scarlight-home)
            ...
            ;;
        -h|--help)
            echo "Scarlight Agent Installer"
            echo ""
            ...
            echo "                   default (non-root):  ~/.scarlight/scarlight-agent"
            echo "                   default (root, Linux): /usr/local/lib/scarlight-agent"
            echo "  --scarlight-home PATH  Data directory (default: ~/.scarlight, or \$SCARLIGHT_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Scarlight installs the code under"
            echo "  /usr/local/lib/scarlight-agent and links the command into"
            echo "  /usr/local/bin/scarlight (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SCARLIGHT_HOME"
            echo "  (default /root/.scarlight).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SCARLIGHT_HOME/scarlight-agent are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `scarlight` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SCARLIGHT_HOME/scarlight-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SCARLIGHT_HOME/scarlight-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/scarlight-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SCARLIGHT_HOME/scarlight-agent — then preserve it)
#
# Always no-op when the user set --dir or $SCARLIGHT_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SCARLIGHT_HOME.
    if is_termux; then
        INSTALL_DIR="$SCARLIGHT_HOME/scarlight-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SCARLIGHT_HOME/scarlight-agent/.git" ]; then
            INSTALL_DIR="$SCARLIGHT_HOME/scarlight-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/scarlight-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/scarlight-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/scarlight"
        log_info "  Data:    $SCARLIGHT_HOME (unchanged)"
        return 0
    fi
...
get_scarlight_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/scarlight" ]; then
        echo "$link_dir/scarlight"
    else
        echo "scarlight"
    fi
}
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/scarlight-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/scarlight-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$SCARLIGHT_HOME/node/bin/node" ]; then
        export PATH="$SCARLIGHT_HOME/node/bin:$PATH"
        local found_ver=$("$SCARLIGHT_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Scarlight-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.scarlight/node/..."
    ...
    # Place into ~/.scarlight/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$SCARLIGHT_HOME/node"
    mkdir -p "$SCARLIGHT_HOME"
    mv "$extracted_dir" "$SCARLIGHT_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$SCARLIGHT_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$SCARLIGHT_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$SCARLIGHT_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$SCARLIGHT_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$SCARLIGHT_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.scarlight/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Scarlight install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Scarlight Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="scarlight-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Scarlight behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"scarlight-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up scarlight command..."

    if [ "$USE_VENV" = true ]; then
        SCARLIGHT_BIN="$INSTALL_DIR/venv/bin/scarlight"
    else
        HERMES_BIN="$(which hermes 2>/dev/null || echo "")"
        if [ -z "$SCARLIGHT_BIN" ]; then
            log_warn "scarlight not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$SCARLIGHT_BIN" ]; then
        log_warn "scarlight entry point not found at $SCARLIGHT_BIN"
    ...
    # Create a user-facing shim for the scarlight command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    cat > "$command_link_dir/scarlight" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SCARLIGHT_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/scarlight"
    log_success "Installed scarlight launcher → $command_link_display_dir/scarlight"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "scarlight command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v scarlight' \
                >/dev/null 2>&1; then
        ...
        log_info "scarlight not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Scarlight Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Scarlight Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Scarlight Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so scarlight works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.scarlight directory structure (config at top level, code in subdir)
    mkdir -p "$SCARLIGHT_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.scarlight/.env (top level, easy to find)
    if [ ! -f "$SCARLIGHT_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SCARLIGHT_HOME/.env"
            log_success "Created ~/.scarlight/.env from template"
        else
            touch "$SCARLIGHT_HOME/.env"
            log_success "Created ~/.scarlight/.env"
        fi
    else
        log_info "~/.scarlight/.env already exists, keeping it"
    fi
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.scarlight/config.yaml (top level, easy to find)
    if [ ! -f "$SCARLIGHT_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SCARLIGHT_HOME/config.yaml"
            log_success "Created ~/.scarlight/config.yaml from template"
        fi
    else
        log_info "~/.scarlight/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$SCARLIGHT_HOME/SOUL.md" ]; then
        cat > "$SCARLIGHT_HOME/SOUL.md" << 'SOUL_EOF'
# Scarlight Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Scarlight communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.scarlight/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.scarlight/"

    # Seed bundled skills into ~/.scarlight/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.scarlight/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.scarlight/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SCARLIGHT_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$SCARLIGHT_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.scarlight/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$SCARLIGHT_HOME/.env"
    ...
    {
        echo ""
        echo "# Scarlight Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Scarlight will use the system browser."
        else
            case "$DISTRO" in
                ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                    log_info "Playwright may request sudo to install browser system dependencies (shared libraries)."
                    log_info "This is standard Playwright setup — Scarlight itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (scarlight --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'scarlight setup' after install."
        return 0
    fi
    ...
    # Run scarlight setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m scarlight_cli.main setup < /dev/tty
    else
        python -m scarlight_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$SCARLIGHT_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Scarlight to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SCARLIGHT_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'scarlight whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SCARLIGHT_CMD="$(get_scarlight_command_path)"
                $SCARLIGHT_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'scarlight whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'scarlight gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SCARLIGHT_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SCARLIGHT_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: scarlight gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: scarlight gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SCARLIGHT_CMD gateway > "$SCARLIGHT_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.scarlight/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: scarlight gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: scarlight gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SCARLIGHT_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SCARLIGHT_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SCARLIGHT_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}scarlight${NC}              Start chatting"
    echo -e "   ${GREEN}scarlight setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}scarlight config${NC}       View/edit configuration"
    echo -e "   ${GREEN}scarlight config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}scarlight gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}scarlight update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'scarlight' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'scarlight' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'scarlight' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
