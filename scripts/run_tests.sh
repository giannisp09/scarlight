#!/usr/bin/env bash
# Canonical test runner for scarlight-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.scarlight/scarlight-agent/venv"; do
...
# Unset SCARLIGHT_* behavioral vars too.
unset SCARLIGHT_YOLO_MODE SCARLIGHT_INTERACTIVE SCARLIGHT_QUIET SCARLIGHT_TOOL_PROGRESS \
      SCARLIGHT_TOOL_PROGRESS_MODE SCARLIGHT_MAX_ITERATIONS SCARLIGHT_SESSION_PLATFORM \
      SCARLIGHT_SESSION_CHAT_ID SCARLIGHT_SESSION_CHAT_NAME SCARLIGHT_SESSION_THREAD_ID \
      SCARLIGHT_SESSION_SOURCE SCARLIGHT_SESSION_KEY SCARLIGHT_GATEWAY_SESSION \
      SCARLIGHT_CRON_SESSION \
      SCARLIGHT_PLATFORM SCARLIGHT_INFERENCE_PROVIDER SCARLIGHT_MANAGED SCARLIGHT_DEV \
      SCARLIGHT_CONTAINER SCARLIGHT_EPHEMERAL_SYSTEM_PROMPT SCARLIGHT_TIMEZONE \
      SCARLIGHT_REDACT_SECRETS SCARLIGHT_BACKGROUND_NOTIFICATIONS SCARLIGHT_EXEC_ASK \
      SCARLIGHT_HOME_MODE 2>/dev/null || true
...
# If a system-wide scarlight pytest_live_guard plugin is installed at
# $HOME/.scarlight/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.scarlight/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.scarlight:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.scarlight" ;;
  esac
...
WORKERS="${SCARLIGHT_TEST_WORKERS:-4}"
