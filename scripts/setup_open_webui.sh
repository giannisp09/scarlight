...
# Bootstrap Open WebUI against Scarlight Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.scarlight/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-scarlight.sh
...
#   OPEN_WEBUI_NAME='Johnny Scarlight'
...
#   SCARLIGHT_API_PORT=8642
#   SCARLIGHT_API_HOST=127.0.0.1
#   SCARLIGHT_API_MODEL_NAME='Scarlight Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Scarlight Agent WebUI}"
...
SCARLIGHT_ENV_FILE="${SCARLIGHT_ENV_FILE:-$HOME/.scarlight/.env}"
SCARLIGHT_API_PORT="${SCARLIGHT_API_PORT:-8642}"
SCARLIGHT_API_HOST="${SCARLIGHT_API_HOST:-127.0.0.1}"
SCARLIGHT_API_CONNECT_HOST="${SCARLIGHT_API_CONNECT_HOST:-127.0.0.1}"
SCARLIGHT_API_MODEL_NAME="${SCARLIGHT_API_MODEL_NAME:-Scarlight Agent}"
SCARLIGHT_API_BASE_URL="http://${SCARLIGHT_API_CONNECT_HOST}:${SCARLIGHT_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-scarlight.sh"
LOG_DIR="$HOME/.scarlight/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$SCARLIGHT_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$SCARLIGHT_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.scarlight.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.scarlight</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.scarlight"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.scarlight"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-scarlight.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Scarlight Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-scarlight.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.scarlight/logs/openwebui.log
StandardError=append:%h/.scarlight/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-scarlight.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$SCARLIGHT_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Scarlight API server is configured...'
  upsert_env API_SERVER_ENABLED true "$SCARLIGHT_ENV_FILE"
  upsert_env API_SERVER_HOST "$SCARLIGHT_API_HOST" "$SCARLIGHT_ENV_FILE"
  upsert_env API_SERVER_PORT "$SCARLIGHT_API_PORT" "$SCARLIGHT_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$SCARLIGHT_API_MODEL_NAME" "$SCARLIGHT_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$SCARLIGHT_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Scarlight gateway so API server settings take effect...'
  scarlight gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${SCARLIGHT_API_CONNECT_HOST}:${SCARLIGHT_API_PORT}/health" >/dev/null; then
    log 'Scarlight API server did not answer on the first check. Trying to start gateway in the background...'
    nohup scarlight gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${SCARLIGHT_API_CONNECT_HOST}:${SCARLIGHT_API_PORT}/health" >/dev/null
  ...
  log "Scarlight API endpoint: ${SCARLIGHT_API_BASE_URL}"
