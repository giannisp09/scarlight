# ============================================================================
# Scarlight Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$ScarlightHome = "$env:LOCALAPPDATA\scarlight",
    [string]$InstallDir = "$env:LOCALAPPDATA\scarlight\scarlight-agent"
)
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Scarlight Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Install-Git {
    ...
    which Scarlight uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\scarlight\git`` — never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Scarlight needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Scarlight copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\scarlight\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SCARLIGHT_GIT_BASH_PATH`` (User scope) so Scarlight can find it in a fresh
    ...
    # Download PortableGit into $ScarlightHome\git.  Always works as long as
    # we can reach github.com — no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found — downloading PortableGit to $ScarlightHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "scarlight-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected — PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Scarlight features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Scarlight needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SCARLIGHT_GIT_BASH_PATH`` (User env scope) so Scarlight can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ScarlightHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ScarlightHome\git\usr\bin\bash.exe
    $candidates += "$ScarlightHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ScarlightHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SCARLIGHT_GIT_BASH_PATH", $candidate, "User")
            $env:SCARLIGHT_GIT_BASH_PATH = $candidate
            Write-Info "Set SCARLIGHT_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe — Scarlight may not find Git Bash."
    Write-Info "If needed, set SCARLIGHT_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$ScarlightHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$ScarlightHome\node;$env:Path"
        Write-Success "Node.js $version found (Scarlight-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.scarlight/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\scarlight-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ScarlightHome\node") { Remove-Item -Recurse -Force "$ScarlightHome\node" }
                Move-Item $extractedDir.FullName "$ScarlightHome\node"
                $env:Path = "$ScarlightHome\node;$env:Path"

                $version = & "$ScarlightHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.scarlight/node/"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running scarlight processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $zipPath = "$env:TEMP\scarlight-agent-$Branch.zip"
                $extractPath = "$env:TEMP\scarlight-agent-extract"
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install scarlight-agent package even with no extras. Inspect the uv pip install output above."
    }

    # Verify the dashboard deps specifically — they're the most common thing
    # users hit and lazy-import errors from `scarlight dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable — `scarlight dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `scarlight dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up scarlight command..."
    
    if ($NoVenv) {
        $scarlightBin = "$InstallDir"
    } else {
        $scarlightBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so scarlight is globally available
    # On Windows, the scarlight.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$scarlightBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$scarlightBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $scarlightBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SCARLIGHT_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\scarlight instead
    # of the Unix default ~/.scarlight
    $currentScarlightHome = [Environment]::GetEnvironmentVariable("SCARLIGHT_HOME", "User")
    if (-not $currentScarlightHome -or $currentScarlightHome -ne $ScarlightHome) {
        [Environment]::SetEnvironmentVariable("SCARLIGHT_HOME", $ScarlightHome, "User")
        Write-Success "Set SCARLIGHT_HOME=$ScarlightHome"
    }
    $env:SCARLIGHT_HOME = $ScarlightHome
    
    # Update current session
    $env:Path = "$scarlightBin;$env:Path"
    
    Write-Success "scarlight command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.scarlight directory structure
    New-Item -ItemType Directory -Force -Path "$ScarlightHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ScarlightHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ScarlightHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ScarlightHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ScarlightHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ScarlightHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ScarlightHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ScarlightHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ScarlightHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ScarlightHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.scarlight/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.scarlight/.env"
        }
    } else {
        Write-Info "~/.scarlight/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ScarlightHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.scarlight/config.yaml from template"
        }
    } else {
        Write-Info "~/.scarlight/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Scarlight's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$ScarlightHome\SOUL.md"
...
"@
        ...
        Write-Success "Created ~/.scarlight/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.scarlight/"
    
    # Seed bundled skills into ~/.scarlight/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.scarlight/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.scarlight/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ScarlightHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.scarlight/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'scarlight setup tools' later."
        return
    }
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\scarlight-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\scarlight-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
                try {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\scarlight-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.scarlight/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m scarlight_cli.main setup
    } else {
        python -m scarlight_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $scarlightCmd = "$InstallDir\venv\Scripts\scarlight.exe"
    if (-not (Test-Path $scarlightCmd)) {
        $scarlightCmd = "scarlight"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ScarlightHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'scarlight whatsapp' to pair via QR code..."
        Write-Host ""
        $response = Read-Host "Pair WhatsApp now? [Y/n]"
        if ($response -eq "" -or $response -match "^[Yy]") {
            try {
                & $scarlightCmd whatsapp
            } catch {
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ScarlightHome\logs\gateway.log"
            Start-Process -FilePath $scarlightCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ScarlightHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: scarlight gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: scarlight gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ScarlightHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ScarlightHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ScarlightHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ScarlightHome\scarlight-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   scarlight              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   scarlight setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   scarlight config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   scarlight config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   scarlight gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   scarlight update       " -NoNewline -ForegroundColor Green
